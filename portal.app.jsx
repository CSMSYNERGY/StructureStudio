...
function ssClampTab(tab, isOperator, canAdmin) {
  if (tab === "accounts" || tab === "admin") return isOperator ? tab : "designs";
  return (!canAdmin && !NONADMIN_TABS.includes(tab)) ? "designs" : tab;
}
...
function LoginView({ onRecoverySent, notice }) {
  ...
  const forgot = async () => {
    ...
    const { error: err } = await sb.auth.resetPasswordForEmail(email.trim(), {
      redirectTo: "https://structurestudio.app/portal",
    });
    setBusy(false);
...
function SourceChip({ unitId, serial }) {
  const inv = Boolean(unitId);
  return (
    <span title={inv ? "Quoted from a building already on the lot" : "Designed for this customer"}
...
function DesignsTable({ clientId, refreshKey = 0, fetchDesigns = null, isAdmin = false, viewingLabel = null, onOpenDesign = null }) {
  ...
  // Chip filter first, then the free-text search over what survives.
  ...
  return (
    <div style={S.card}>
      <CardHead
        title="Designs"
        count={rows ? ((query || statusFilter !== "all") ? `${filtered.length} of ${rows.length}` : rows.length) : null}
        right={<button onClick={load} style={{ ...S.btn("#F1F5F9", "#334155"), border: "1px solid #E2E8F0", padding: "6px 12px" }}>↻ Refresh</button>}
      />
      {rows && rows.length > 0 && <SearchInput value={query} onChange={setQuery} placeholder="Search designs — name, email, phone, building, estimate #…" />}
      ...
      {rows && rows.length > 0 && filtered.length === 0 && (
        <p style={{ fontSize: 13, color: "#64748B", padding: 12 }}>
          {query
            ? <>No designs match “{query}”{statusFilter !== "all" ? <> in <strong>{STATUS_LABELS[statusFilter]}</strong></> : null}.</>
...
function LeadsTable({ clientId, fetchDesigns = null, isAdmin = false, onOpenDesign = null }) {
  ...
  return (
    <div style={S.card}>
      ...
      {rows && rows.length > 0 && <SearchInput value={query} onChange={setQuery} placeholder="Search contacts — name, email, phone, status…" />}
      ...
      {rows && rows.length > 0 && filtered.length === 0 && (
        <p style={{ fontSize: 13, color: "#64748B", padding: 12 }}>
          {query
            ? <>No contacts match “{query}”{statusFilter !== "all" ? <> in <strong>{statusFilter === "browsing" ? "Browsing" : STATUS_LABELS[statusFilter]}</strong></> : null}.</>
...
function BillingView({ viewingLabel = null }) {
  ...
  // Simple Layout is required: preselect it (yearly) until it's subscribed.
  useEffect(() => {
    if (data && data.configured && !baseLive && byFeature["simple_layout"] && !sel["simple_layout"]) {
  ...
  const toggleFeature = (f) => {
    ...
    if (f.required && !baseLive) return;   // Simple Layout stays in the cart
    setSel((p) => { const n = { ...p }; if (n[f.feature]) delete n[f.feature]; else n[f.feature] = "annual"; return n; });
  };
        ...
        <div style={S.card}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(230px, 1fr))", gap: 12 }}>
            {features.filter((f) => !liveFeatures[f.feature]).map((f) => {
              ...
              return (
                <div key={f.feature} onClick={() => toggleFeature(f)}
                  style={{ position: "relative", border: on ? `2px solid ${ACCENT}` : "1px solid #E2E8F0", borderRadius: 10, padding: on ? 15 : 16, cursor: soon ? "default" : "pointer", overflow: "hidden" }}>
                  {soon && (
                    <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center", zIndex: 2, pointerEvents: "none" }}>
                      <span style={{ transform: "rotate(-12deg)", background: "#FEF3C7", color: "#B45309", border: "1px solid #FDE68A", borderRadius: 8, padding: "6px 16px", fontSize: 13, fontWeight: 800, letterSpacing: 1.5, textTransform: "uppercase", whiteSpace: "nowrap" }}>Coming soon</span>
                    </div>
                  )}
                  <div style={{ opacity: soon ? 0.45 : 1 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap", marginBottom: 6 }}>
                      ...
                      {!soon && !lockedBase && <span style={{ marginLeft: "auto", fontSize: 18, color: on ? ACCENT : "#CBD5E1", fontWeight: 800 }}>{on ? "✓" : "+"}</span>}
                    </div>
                    {!soon && (
...
function PricingCsv({ viewingLabel = null }) {
...
function FixtureCatalog({ category, noun, addLabel, namePh, labelPh, wPh, hPh, sizeWord = "height", hasSwingOp = false, viewingLabel = null, clientId = null }) {
  ...
  const star = (on, onClick) => <span onClick={onClick} title="Default the customer sees first" style={{ cursor: "pointer", color: on ? "#F59E0B" : "#CBD5E1", fontSize: 15 }}>★</span>;
...
function ComingSoon({ icon, title, blurb, bullets, cta }) {
  return (
    <div style={{ ...S.card, padding: 0, overflow: "hidden", maxWidth: 720 }}>
      <div style={{ background: "linear-gradient(135deg, #3D3672 0%, #1B7895 100%)", padding: "34px 28px", color: "#FFF", display: "flex", alignItems: "center", gap: 20 }}>
        ...
        <div style={{ minWidth: 0 }}>
          <span style={{ display: "inline-block", background: "#75E6DA", color: "#22345B", fontSize: 10.5, fontWeight: 800, letterSpacing: 0.6, textTransform: "uppercase", borderRadius: 999, padding: "3px 10px", marginBottom: 9 }}>{cta ? "Available now" : "Coming soon"}</span>
...
function OrdersView({ clientId }) {
  ...
  return (
    <div>
      ...
      <div style={S.card}>
        <div style={{ display: "flex", alignItems: "center", gap: 9, flexWrap: "wrap", marginBottom: 12 }}>
          <div style={{ ...S.h2, marginBottom: 0, marginRight: 4 }}>Orders {rows ? (query || filter !== "all" ? `(${shown.length} of ${all.length})` : `(${all.length})`) : ""}</div>
        ...
        {all.length > 0 && shown.length === 0 && (
          <p style={{ fontSize: 13, color: "#64748B", padding: 12 }}>No orders match this filter.</p>
        )}
        {shown.length > 0 && (
          <div style={{ overflowX: "auto" }}>
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              <thead><tr>
                ...
                <th style={{ ...S.th, textAlign: "right" }}>Balance</th><th style={S.th}>Status</th><th style={S.th}></th>
              </tr></thead>
              <tbody>
                {shown.map((r) => {
...
// A unit displays Sold when explicitly marked, OR when any estimate sent from it has been
// accepted (or further) — a sale recorded only in the CRM still flips the row here.
const INV_SOLD_EST = new Set(["accepted", "invoiced", "delivered"]);
const invEffStatus = (u) =>
  (u.status === "sold" || (u.estimates || []).some((e) => INV_SOLD_EST.has(normStatus(e.status)))) ? "sold" : "available";
function InventoryTable({ clientId, refreshKey = 0, isAdmin = false, onOpenDesign = null, onSendEstimate = null, onNew = null }) {
  const [rows, setRows] = useState(null);      // null = loading
  ...
  const [expanded, setExpanded] = useState({});  // unit id -> bool
  ...
  const load = useCallback(async () => {
    ...
    // list_inventory reports each estimate's CACHED designs.status, and nothing else on
    // this tab talks to GHL — so a customer accepting in the CRM would not flip the
    // building to Sold until somebody happened to open the Designs tab. Refresh the
    // linked estimates here and re-derive. Additive: any failure just leaves the cache.
    ...
    if (codes.length) {
      try {
        ...
        if (statuses) {
          setRows(units.map((u) => ({
            ...u,
            estimates: (u.estimates || []).map((e) => statuses[e.shortCode] ? { ...e, status: statuses[e.shortCode] } : e),
          })));
        }
      } catch (_e) { /* cached statuses stand */ }
    }
  }, [clientId]);
  useEffect(() => { load(); }, [load, refreshKey]);

  const counts = (rows || []).reduce((a, u) => { const k = invEffStatus(u); a[k] = (a[k] || 0) + 1; return a; }, {});
  const filtered = (rows || []).filter((u) => {
    if (statusFilter !== "all" && invEffStatus(u) !== statusFilter) return false;
    ...
    const hay = ["#" + u.serial, String(u.serial), titleCase(u.style || ""), u.size || "", u.locationName || "",
      u.roofType || "", u.roofColor || "", u.bodyColor || "", u.trimColor || "",
  ...
  const markStatus = async (u, to) => {
    setBusyId(u.id); setMsg(null);
    await invoke({ action: "update_inventory", unitId: u.id, status: to },
      to === "sold" ? `#${u.serial} marked sold.` : `#${u.serial} is available again.`);
    setBusyId(null); load();
  };
  const del = async (u) => {
  ...
  const chip = (k, label, n, on) => (
    <button key={k} type="button" aria-pressed={on} onClick={() => setStatusFilter(k)}
  ...
  return (
    <div style={S.card}>
      <CardHead title="Inventory" count={total}
        right={<>
          <button type="button" onClick={load} style={S.btn("#F1F5F9", "#334155")}>↻ Refresh</button>
          {onNew && <button type="button" onClick={onNew} style={S.btn(ACCENT, "#FFF")}>+ New inventory building</button>}
        </>} />
      ...
      <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 13, alignItems: "center" }}>
        {chip("all", "All", total, statusFilter === "all")}
        {(counts.available || 0) > 0 && chip("available", "Available", counts.available, statusFilter === "available")}
        {(counts.sold || 0) > 0 && chip("sold", "Sold", counts.sold, statusFilter === "sold")}
        {locations.length > 0 && (
          <select value={locFilter} onChange={(e) => setLocFilter(e.target.value)}
            style={{ marginLeft: "auto", fontSize: 12, fontWeight: 700, color: "#334155", border: "1px solid #E2E8F0", background: "#FFF", borderRadius: 9, padding: "6px 10px" }}>
      ...
      {rows && rows.length === 0 && !error && (
        <p style={{ fontSize: 13, color: "#64748B", padding: 12 }}>
          Nothing on the lot yet. Design a building in the <strong>Designer</strong> tab and click <strong>Save to Inventory</strong> — it takes the next serial number automatically.
        </p>
      )}
      {rows && rows.length > 0 && filtered.length === 0 && (
        <p style={{ fontSize: 13, color: "#64748B", padding: 12 }}>No buildings match{query ? <> “{query}”</> : null}.</p>
      )}
      {rows && filtered.length > 0 && (
        <div style={{ overflowX: "auto" }}>
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            ...
            <tbody>
              {filtered.map((u) => {
                const st = invEffStatus(u);
                const ests = u.estimates || [];
                const isOpen = !!expanded[u.id];
                const winner = ests.find((e) => INV_SOLD_EST.has(normStatus(e.status)));
                return (
                  <React.Fragment key={u.id}>
                    <tr>
                      ...
                      <td style={S.td}>
                        <span style={{ background: st === "sold" ? "#EEF2FF" : "#F0FDF4", color: st === "sold" ? "#3D3672" : "#15803D", borderRadius: 20, padding: "4px 12px", fontSize: 12, fontWeight: 700, whiteSpace: "nowrap" }}>
                          {st === "sold" ? "Sold" : "Available"}
                        </span>
                      </td>
                      <td style={{ ...S.td, whiteSpace: "nowrap" }}>
                        <button type="button" disabled={busyId === u.id}
                          onClick={() => onOpenDesign && onOpenDesign(u.shortCode, null, { unit: { unitId: u.id, askingPriceCents: u.askingPriceCents, locationId: u.locationId } })}
                          style={{ background: "none", border: "none", padding: 0, cursor: "pointer", fontFamily: "inherit", fontSize: "inherit", color: ACCENT, fontWeight: 700, marginRight: 10 }}>Open</button>
                        {st !== "sold" && (
                          <button type="button" disabled={busyId === u.id}
                            onClick={() => onSendEstimate && onSendEstimate(u)}
                            style={{ background: "none", border: "none", padding: 0, cursor: "pointer", fontFamily: "inherit", fontSize: "inherit", color: ACCENT, fontWeight: 700, marginRight: 10 }}>Send estimate</button>
                        )}
                        ...
                        {isAdmin && (u.status === "sold"
                          ? <button type="button" disabled={busyId === u.id} onClick={() => markStatus(u, "available")}
                              style={{ background: "none", border: "none", padding: 0, cursor: "pointer", fontFamily: "inherit", fontSize: "inherit", color: "#64748B", fontWeight: 700, marginRight: 10 }}>Mark available</button>
                          : (st !== "sold" && <button type="button" disabled={busyId === u.id} onClick={() => markStatus(u, "sold")}
                              style={{ background: "none", border: "none", padding: 0, cursor: "pointer", fontFamily: "inherit", fontSize: "inherit", color: "#64748B", fontWeight: 700, marginRight: 10 }}>Mark sold</button>))}
                        {isAdmin && <button type="button" disabled={busyId === u.id} onClick={() => del(u)}
                          style={{ background: "none", border: "none", padding: 0, cursor: "pointer", fontFamily: "inherit", fontSize: "inherit", color: "#CBD5E1", fontWeight: 700 }}>Delete</button>}
                      </td>
                    </tr>
                            ...
                            return (
                              <div key={e.shortCode} style={{ display: "flex", alignItems: "center", gap: 10, padding: "6px 0", fontSize: 13, flexWrap: "wrap" }}>
                                ...
                                <span style={{ background: c.bg, color: c.fg, borderRadius: 14, padding: "2.5px 10px", fontSize: 11, fontWeight: 700 }}>
                                  {STATUS_LABELS[es]}{isWinner && st === "sold" ? " — buyer" : ""}
                                </span>
                                {st === "sold" && !isWinner && !INV_SOLD_EST.has(es) && (
                                  <span style={{ background: "#FEF3C7", color: "#92400E", borderRadius: 12, fontSize: 11, fontWeight: 700, padding: "3px 10px" }}>⚠ Building sold — offer a new build?</span>
                                )}
...
// Past due = has a due date, that date is before today, and the stage isn't finished.
const schedPastDue = (job, kind) => {
  if (!job.due_date || kind === "done") return false;
  return job.due_date < new Date().toISOString().slice(0, 10);
};
...
// never in a popout (scope decision 6). Admin edits fields; every role can add a note.
function SchedJobEditor({ job, stages, team, canAdmin, busy, onSave, onComplete, onDelete, onMove, onNote, onOpenDesign }) {
  const [f, setF] = useState({
    title: job.title || "", customerName: job.customer_name || "", buildingLabel: job.building_label || "",
    scheduledStart: job.scheduled_start || "", dueDate: job.due_date || "",
    assigneeUserId: job.assignee_user_id || "", notes: job.notes || "",
  });
    ...
    <div style={{ borderTop: "1px solid #F1F5F9", marginTop: 8, paddingTop: 9 }} onClick={(e) => e.stopPropagation()}>
      {canAdmin && (<>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "8px 10px", marginBottom: 8 }}>
          ...
          <div><span style={lbl}>Start</span><input type="date" style={inp} value={f.scheduledStart} onChange={set("scheduledStart")} /></div>
          <div><span style={lbl}>Due</span><input type="date" style={inp} value={f.dueDate} onChange={set("dueDate")} /></div>
          <div><span style={lbl}>Assigned to</span>
            <select style={inp} value={f.assigneeUserId} onChange={set("assigneeUserId")}>
              <option value="">— Unassigned —</option>
              {team.map((m) => <option key={m.userId} value={m.userId}>{m.name || m.userId.slice(0, 8)}</option>)}
            </select>
          </div>
          <div><span style={lbl}>Job notes</span><input style={inp} value={f.notes} onChange={set("notes")} placeholder="Shown on the card" /></div>
        </div>
        <div style={{ display: "flex", gap: 7, flexWrap: "wrap", marginBottom: 8 }}>
          <button type="button" disabled={busy} style={S.btn(ACCENT, "#FFF")} onClick={() => onSave({
            ...
            scheduledStart: f.scheduledStart || null, dueDate: f.dueDate || null,
            assigneeUserId: f.assigneeUserId || null, notes: f.notes,
          })}>Save</button>
      ...
      <div style={{ display: "flex", gap: 7, alignItems: "center", flexWrap: "wrap" }}>
        ...
        <button type="button" disabled={busy || !note.trim()} style={S.btn("#F1F5F9", "#334155")}
          onClick={() => { if (note.trim()) { onNote(note.trim()); setNote(""); } }}>Add note</button>
      </div>
    </div>
  );
}
...
function BuildScheduleTab({ clientId, canAdmin, onOpenDesign }) {
  ...
  const jobCard = (job) => {
    ...
    return (
      <div key={job.id} draggable={!open} tabIndex={0} role="button" aria-expanded={open}
        ...
        <div style={{ fontSize: 13, fontWeight: 800, lineHeight: 1.25 }}>{job.customer_name || job.title || "—"}</div>
        {(job.building_label || job.title) && job.customer_name && (
          <div style={{ fontSize: 11.5, fontWeight: 600, color: "#64748B", marginTop: 1 }}>{job.building_label || job.title}</div>
        )}
        <div style={{ display: "flex", alignItems: "center", gap: 6, marginTop: 7, flexWrap: "wrap" }}>
          {(job.scheduled_start || job.due_date) && (
            <span style={{ ...schedChip(late ? "#FEF2F2" : "#F1F5F9", late ? "#DC2626" : "#475569"), fontVariantNumeric: "tabular-nums" }}>
              {job.scheduled_start ? fmtDate(job.scheduled_start) : ""}{job.scheduled_start && job.due_date ? " → " : ""}{job.due_date ? fmtDate(job.due_date) : ""}{late ? " · past due" : ""}
            </span>
          )}
          {loadChip(job)}
          {job.assignee_user_id && (
            <span title={nameOf[job.assignee_user_id] || ""} style={{ width: 21, height: 21, borderRadius: "50%", background: "#DBEAFF", border: "1px solid #C3D9F7", color: ACCENT, display: "inline-flex", alignItems: "center", justifyContent: "center", fontSize: 9, fontWeight: 800, marginLeft: "auto", flexShrink: 0 }}>
        ...
        {open && (
          <SchedJobEditor job={job} stages={stages} team={team} canAdmin={canAdmin} busy={busy}
  ...
  const trayItem = (key, chipConf, who, what, onAdd) => (
    <div key={key} style={{ flex: "0 0 auto", display: "flex", alignItems: "center", gap: 9, border: "1px solid #E2E8F0", borderRadius: 10, padding: "8px 11px", background: "#FBFCFE" }}>
      ...
      {canAdmin && (
        <button type="button" disabled={busy} onClick={onAdd} title="Add to the board"
    ...
    <div style={S.card}>
      <CardHead title="Build Schedule" count={jobs.length}
        right={<>
          ...
          <button type="button" onClick={load} style={S.btn("#F1F5F9", "#334155")}>↻ Refresh</button>
          {canAdmin && <button type="button" onClick={() => setEditingStages(true)} style={S.btn("#FFF", "#475569")}>Edit stages</button>}
          {canAdmin && <button type="button" onClick={() => setAddOpen(!addOpen)} style={S.btn(ACCENT, "#FFF")}>+ Add job</button>}
        </>} />
      ...
      {/* Unscheduled tray — sold orders / available inventory / open repairs with no card yet */}
      {data && trayCount > 0 && (
        <div style={{ border: "1px dashed #B9C4D6", borderRadius: 12, padding: "10px 14px", marginBottom: 13, background: "#FFF" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 9 }}>
            <span style={{ ...S.lbl, marginBottom: 0, color: "#475569" }}>Unscheduled</span>
            <span style={schedChip("#EEF2FF", "#3D3672")}>{trayCount}</span>
            <span style={{ fontSize: 11.5, color: "#64748B", fontWeight: 600 }}>— sold orders, inventory, and repairs that aren't on the board yet</span>
          </div>
          <div style={{ display: "flex", gap: 9, overflowX: "auto", paddingBottom: 2 }}>
            {tray.orders.map((o) => trayItem("o" + o.designShortCode, SCHED_SRC_CHIP.order, o.customerName || o.designShortCode, o.buildingLabel,
              () => addFromTray({ source: "order", designShortCode: o.designShortCode }, o.customerName || "Order")))}
            {tray.inventory.map((u) => trayItem("u" + u.inventoryUnitId, SCHED_SRC_CHIP.inventory, "#" + u.serial, "Spec build for the lot",
              () => addFromTray({ source: "inventory", inventoryUnitId: u.inventoryUnitId }, "#" + u.serial)))}
            {tray.repairs.map((rp) => trayItem("r" + rp.repairId, SCHED_SRC_CHIP.repair, rp.customerName, "R-" + rp.repairNo + " · " + (rp.description || "").slice(0, 60),
              () => addFromTray({ source: "repair", repairId: rp.repairId }, "R-" + rp.repairNo)))}
          </div>
        </div>
      )}
        ...
        <div style={{ overflowX: "auto" }}>
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            <thead><tr>
              ...
              <SortTh label="Start" col="scheduled_start" sortKey={sortKey} sortDir={sortDir} onSort={onSort} />
              <SortTh label="Due" col="due_date" sortKey={sortKey} sortDir={sortDir} onSort={onSort} />
                ...
                return (
                  <React.Fragment key={j.id}>
                    <tr onClick={() => setExpandedId(open ? null : j.id)} style={{ cursor: "pointer", background: open ? "#F7F9FF" : "transparent" }}>
                      ...
                      <td style={{ ...S.td, whiteSpace: "nowrap" }}>{j.scheduled_start ? fmtDate(j.scheduled_start) : "—"}</td>
                      <td style={{ ...S.td, whiteSpace: "nowrap", color: late ? "#DC2626" : "#1E293B", fontWeight: late ? 700 : 400 }}>{j.due_date ? fmtDate(j.due_date) + (late ? " · past due" : "") : "—"}</td>
                      <td style={S.td}>{nameOf[j.assignee_user_id] || "—"}</td>
                      <td style={S.td}>{loadChip(j) || <span style={{ fontSize: 11.5, color: "#94A3B8", fontWeight: 600 }}>not on a load</span>}</td>
                    </tr>
                    {open && (
                      <tr><td colSpan={10} style={{ ...S.td, background: "#F7F9FF" }}>
                        <SchedJobEditor job={j} stages={stages} team={team} canAdmin={canAdmin} busy={busy}
...
function RepairsTab({ clientId, canAdmin }) {
  ...
  const createRepair = async () => {
    ...
    const r = await call({
      ...
      serial: intake.serial.trim() ? Number(intake.serial.replace(/[^0-9]/g, "")) : null,
  ...
  return (
    <div style={S.card}>
      <CardHead title="Repairs" count={repairs.length}
        right={<>
          <button type="button" onClick={load} style={S.btn("#F1F5F9", "#334155")}>↻ Refresh</button>
          {canAdmin && <button type="button" onClick={() => setIntakeOpen(!intakeOpen)} style={S.btn(ACCENT, "#FFF")}>+ Log a repair</button>}
        </>} />
      ...
      {intakeOpen && canAdmin && (
        <div style={{ border: "1px dashed #B9C4D6", borderRadius: 12, padding: "12px 14px", marginBottom: 13 }}>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(170px, 1fr))", gap: "8px 12px", marginBottom: 8 }}>
            ...
            <div><span style={lbl}>Building serial # <span style={{ fontWeight: 500, textTransform: "none" }}>(blank if not one of ours)</span></span><input style={S.input} value={intake.serial} onChange={(e) => setIntake({ ...intake, serial: e.target.value })} placeholder="e.g. 11640" /></div>
            ...
            <div style={{ background: "#FFF", border: "1px solid #E2E8F0", borderRadius: 12, padding: "14px 16px" }}>
              ...
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap", margin: "5px 0 11px" }}>
                {selJob
                  ...
                  : (canAdmin && <button type="button" disabled={busy} style={{ ...S.btn("#FFF", "#475569"), border: "1px solid #CBD5E1", padding: "6px 11px", fontSize: 12 }} onClick={scheduleShopWork}>Schedule shop work →</button>)}
                {selStops.length > 0
                  ? <span style={schedChip("#EEF2FF", "#3D3672")}>{selStops.some((s) => s.delivered_at) ? "Site visit done ✓" : "Site visit on a load ✓"}</span>
                  : <span style={{ fontSize: 11.5, color: "#94A3B8", fontWeight: 600, alignSelf: "center" }}>Site visits ride a delivery load — coming with the Delivery Schedule.</span>}
              </div>
      ...
      <p style={{ fontSize: 11.5, color: "#64748B", fontWeight: 600, lineHeight: 1.5, marginTop: 12 }}>
        Log a repair once, then schedule the work where it happens — shop repairs go on the Build Schedule; site visits will ride a delivery load. Service history follows the building's serial number forever.
      </p>
    </div>
  );
}
...
function DeliveryScheduleTab({ clientId, canAdmin }) {
  ...
  const [msg, setMsg] = useState(null);
  const [busy, setBusy] = useState(false);
  const [newLoad, setNewLoad] = useState(null);          // { driverUserId, loadDate, routeLabel } | null
  ...
  const team = (data && data.team) || [];
  const nameOf = {}; team.forEach((m) => { nameOf[m.userId] = m.name; });
  const driverOf = {}; drivers.forEach((p) => { driverOf[p.user_id] = p; });
  ...
  const visibleLoads = loads.filter((l) => driverFilter === "all" || l.driver_user_id === driverFilter);
  ...
  const createLoad = async (fields) => {
    setBusy(true); setMsg(null);
    const r = await call({ action: "create_load", driverUserId: fields.driverUserId || null, loadDate: fields.loadDate || null, routeLabel: fields.routeLabel || null });
    setBusy(false);
  ...
  const driverLabel = (uid) => {
    const p = driverOf[uid];
    if (!uid) return "No driver";
    return (nameOf[uid] || "Driver") + (p && p.truck_name ? " · " + p.truck_name : "");
  };
  const builtCell = (s, l) => {
    ...
    if (s.source === "inventory") return <span style={schedChip("#ECFEFF", "#0E7490")}>On lot — ready</span>;
  ...
  const poolRow = (key, src, serial, customer, building, size, wide, ready, extraSub, addBody, label) => (
    <tr key={key}>
      <td style={{ ...S.td, whiteSpace: "nowrap" }}>
      ...
      <td style={{ ...S.td, whiteSpace: "nowrap" }}>
        {canAdmin && (
          <select disabled={busy} value="" onChange={(e) => { if (e.target.value) addToLoad(e.target.value, addBody, label); }}
            ...
            {plannedLoads.map((l) => <option key={l.id} value={l.id}>Load {l.load_no}{l.load_date ? " · " + schedDayLabel(l.load_date) : ""}{l.driver_user_id ? " · " + (nameOf[l.driver_user_id] || "") : ""}</option>)}
    ...
    <div style={S.card}>
      <CardHead title="Delivery Schedule" count={loads.length}
        right={<>
          ...
          <button type="button" onClick={load} style={S.btn("#F1F5F9", "#334155")}>↻ Refresh</button>
          {canAdmin && <button type="button" onClick={() => setNewLoad(newLoad ? null : { driverUserId: "", loadDate: "", routeLabel: "" })} style={S.btn(ACCENT, "#FFF")}>+ New load</button>}
        </>} />
      ...
      {newLoad && canAdmin && (
        <div style={{ display: "flex", gap: 10, flexWrap: "wrap", alignItems: "flex-end", border: "1px dashed #B9C4D6", borderRadius: 10, padding: "10px 12px", marginBottom: 12 }}>
          <div style={{ minWidth: 180 }}>
            ...
            <select style={inp} value={newLoad.driverUserId} onChange={(e) => setNewLoad({ ...newLoad, driverUserId: e.target.value })}>
              <option value="">— Pick later —</option>
              {drivers.filter((p) => p.is_driver && p.active).map((p) => <option key={p.user_id} value={p.user_id}>{driverLabel(p.user_id)}</option>)}
            </select>
      ...
      {data && pool && view === "loads" && (<>
        ...
        {poolCount > 0 && (
          <div style={{ border: "1px dashed #B9C4D6", borderRadius: 12, padding: "8px 12px 2px", marginBottom: 14, overflowX: "auto" }}>
            ...
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              <thead><tr>{["Serial", "Customer", "Building", "Size", "Ready", ""].map((h) => <th key={h} style={S.th}>{h}</th>)}</tr></thead>
              <tbody>
                {poolJobs.map((j) => poolRow("j" + j.buildJobId, SCHED_SRC_CHIP[j.source] || SCHED_SRC_CHIP.manual,
                  j.serial ? "#" + j.serial : "—",
                ...
                {poolInv.map((u) => poolRow("u" + u.inventoryUnitId, SCHED_SRC_CHIP.inventory,
                  "#" + u.serial, u.location ? u.location.name : "Sales lot", "Sold lot building", "—", false,
                  <span style={schedChip("#ECFEFF", "#0E7490")}>On lot — ready</span>,
                  "sold — deliver to the buyer", { source: "inventory", inventoryUnitId: u.inventoryUnitId }, "#" + u.serial))}
                {poolRepairs.map((rp) => poolRow("r" + rp.id, SCHED_SRC_CHIP.repair,
                  ...
                  rp.serial ? "building #" + rp.serial : null, { source: "repair", repairId: rp.id }, "R-" + rp.repair_no))}
              </tbody>
            </table>
          </div>
        )}
        ...
        {drivers.filter((p) => p.is_driver).length > 0 && (
          <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 6 }}>
            {[["all", `All drivers (${loads.length})`]].concat(drivers.filter((p) => p.is_driver).map((p) => [p.user_id, driverLabel(p.user_id) + ` (${loads.filter((l) => l.driver_user_id === p.user_id).length})`])).map(([k, label]) => {
                ...
                <div key={l.id} style={{ background: "#FFF", border: "1px solid " + (conflict ? "#FCA5A5" : "#E2E8F0"), borderRadius: 12, marginBottom: 12, overflow: "hidden" }}>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 11, padding: "10px 14px", background: "#F8FAFF", borderBottom: "1px solid #E2E8F0", flexWrap: "wrap", boxShadow: conflict ? "inset 3px 0 0 #DC2626" : "none" }}>
                    ...
                    <span style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: 7 }}>
                      ...
                      {canAdmin && l.status === "planned" && <button type="button" style={{ ...S.btn("#FFF", "#475569"), border: "1px solid #CBD5E1", padding: "4px 10px", fontSize: 11.5 }} onClick={() => setEditLoadId(editLoadId === l.id ? null : l.id)}>{editLoadId === l.id ? "Close" : "Edit"}</button>}
                      {canAdmin && l.status === "planned" && <button type="button" disabled={busy} style={{ ...S.btn("#FFFBEB", "#B45309"), padding: "4px 10px", fontSize: 11.5 }} onClick={() => markLoad(l, false)}>Mark out →</button>}
                      {canAdmin && l.status !== "delivered" && <button type="button" disabled={busy} style={{ ...S.btn("#F0FDF4", "#15803D"), padding: "4px 10px", fontSize: 11.5 }} onClick={() => markLoad(l, true)}>✓ Delivered</button>}
                      {canAdmin && l.status === "planned" && lStops.length === 0 && <button type="button" disabled={busy} style={{ ...S.btn("#FEF2F2", "#DC2626"), padding: "4px 10px", fontSize: 11.5 }} onClick={() => deleteLoad(l)}>Delete</button>}
                    </span>
                  </div>

                  {/* Header edit (in place) */}
                  {editLoadId === l.id && canAdmin && (
                            ...
                            <React.Fragment key={s.id}>
                              <tr style={{ background: stopConflict(s, l) ? "#FFF8F8" : "transparent" }}>
                                ...
                                <td style={{ ...S.td, whiteSpace: "nowrap", textAlign: "right" }}>
                                  {l.status === "out" && !s.delivered_at && <button type="button" disabled={busy} style={{ ...S.btn("#F0FDF4", "#15803D"), padding: "3px 8px", fontSize: 11 }} onClick={() => markStopDelivered(s)}>✓ Delivered</button>}
                                  {canAdmin && l.status === "planned" && (<>
              ...
              {stops.slice().sort((a, b) => {
                ...
                return (
                  <tr key={s.id}>
                    ...
                    <td style={S.td}>{nameOf[l.driver_user_id] || "—"}</td>
                  </tr>
                );
              })}
            </tbody>
      ...
      {data && view === "calendar" && (() => {
        // Monday-start week, shifted by weekOffset.
        const base = new Date(); base.setHours(0, 0, 0, 0);
        base.setDate(base.getDate() - ((base.getDay() + 6) % 7) + weekOffset * 7);
        const days = [];
        for (let i = 0; i < 7; i++) {
          const d = new Date(base.getTime() + i * 86400000);
          days.push({ iso: d.toISOString().slice(0, 10), d });
        }
        const rangeLabel = days[0].d.toLocaleDateString("en-US", { month: "short", day: "numeric" }) + " – " +
        ...
        return (
          <div>
            <div style={{ display: "flex", alignItems: "center", gap: 9, marginBottom: 11 }}>
              <button type="button" style={S.btn("#F1F5F9", "#334155")} onClick={() => setWeekOffset(weekOffset - 1)}>‹ Prev</button>
              <span style={{ fontSize: 13.5, fontWeight: 800 }}>{rangeLabel}</span>
              {weekOffset !== 0 && <button type="button" style={{ ...S.btn("#FFF", "#475569"), border: "1px solid #CBD5E1", padding: "6px 11px", fontSize: 12 }} onClick={() => setWeekOffset(0)}>This week</button>}
              <button type="button" style={S.btn("#F1F5F9", "#334155")} onClick={() => setWeekOffset(weekOffset + 1)}>Next ›</button>
              {undated > 0 && <span style={{ fontSize: 11.5, color: "#64748B", fontWeight: 600, marginLeft: "auto" }}>{undated} planned load{undated === 1 ? "" : "s"} with no date yet — see the Loads view</span>}
            </div>
            <div style={{ overflowX: "auto" }}>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(7, minmax(128px, 1fr))", gap: 8, minWidth: 920 }}>
                {days.map(({ iso, d }) => {
                  const isToday = iso === todayIso;
                  const dayLoads = visibleLoads.filter((l) => l.load_date === iso);
                  return (
                    <div key={iso} style={{ background: isToday ? "#F4F7FF" : "#F8FAFC", border: "1px solid " + (isToday ? "#C3D9F7" : "#E2E8F0"), borderRadius: 10, padding: 8, minHeight: 130 }}>
                      <div style={{ fontSize: 10.5, fontWeight: 800, letterSpacing: 0.5, textTransform: "uppercase", color: isToday ? ACCENT : "#64748B", marginBottom: 7 }}>
                      ...
                      <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                        {dayLoads.map((l) => {
                          ...
                          const conflict = loadConflicts[l.id];
                          return (
                            <button key={l.id} type="button" onClick={() => { setDriverFilter("all"); setView("loads"); }}
                              title={`Load ${l.load_no} — open the Loads view`}
                              style={{ textAlign: "left", background: "#FFF", border: "1px solid " + (conflict ? "#FCA5A5" : "#E2E8F0"), borderRadius: 8, padding: "7px 9px", cursor: "pointer", fontFamily: "inherit", boxShadow: "0 1px 2px rgba(15,23,42,.05)" }}>
                              <div style={{ display: "flex", alignItems: "center", gap: 5, marginBottom: 3 }}>
                                <span style={{ fontSize: 12, fontWeight: 800 }}>Load {l.load_no}</span>
                                <span style={{ ...schedChip(st.bg, st.fg), marginLeft: "auto", padding: "2px 7px", fontSize: 9.5 }}>{st.label}</span>
                              </div>
                              <div style={{ fontSize: 10.5, fontWeight: 600, color: "#64748B" }}>
                                {nameOf[l.driver_user_id] || "No driver"} · {lStops.length} stop{lStops.length === 1 ? "" : "s"}
                              </div>
                              <div style={{ display: "flex", gap: 4, marginTop: 4, flexWrap: "wrap" }}>
                                {l.is_wide && <span style={{ ...schedChip("#FEF3C7", "#92400E"), padding: "1px 6px", fontSize: 9 }}>WIDE</span>}
                                {conflict && <span style={{ ...schedChip("#FEF2F2", "#DC2626"), padding: "1px 6px", fontSize: 9 }}>⚠ not built</span>}
                              </div>
                            </button>
                          );
                        })}
                        {dayLoads.length === 0 && <span style={{ fontSize: 10.5, color: "#CBD5E1", fontWeight: 600 }}>—</span>}
                      </div>
                    </div>
                  );
                })}
...
function SchedLoadHeaderEditor({ l, drivers, nameOf, busy, onSave, onCancel }) {
  const [f, setF] = useState({
    driverUserId: l.driver_user_id || "", loadDate: l.load_date || "", routeLabel: l.route_label || "",
  ...
  return (
    <div style={{ margin: "10px 14px", border: "1px solid #E2E8F0", borderRadius: 10, padding: "10px 13px", background: "#FBFCFE" }}>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(140px, 1fr))", gap: "8px 10px" }}>
        <div>
          <span style={lbl}>Driver</span>
          <select style={inp} value={f.driverUserId} onChange={set("driverUserId")}>
      ...
      <div style={{ display: "flex", gap: 8, justifyContent: "flex-end", marginTop: 9 }}>
        ...
        <button type="button" disabled={busy} style={S.btn(ACCENT, "#FFF")}
          onClick={() => onSave({
            driverUserId: f.driverUserId || null, loadDate: f.loadDate || null, routeLabel: f.routeLabel,
...
function DriversTerritoriesCard() {
  const [data, setData] = useState(null);      // { team, drivers, territories }
  const [msg, setMsg] = useState(null);
  const [busy, setBusy] = useState(false);
  ...
  const load = useCallback(async () => {
    ...
    if (error || !d || d.error) { setMsg({ err: (d && d.error) || "Could not load drivers." }); setData({ team: [], drivers: [], territories: [] }); return; }
    setData(d);
    const map = {};
    (d.drivers || []).forEach((p) => {
      map[p.user_id] = {
  ...
  const blank = { isDriver: false, truckName: "", deck: "", maxW: "", wide: true, territoryIds: [] };
  const draftFor = (uid) => drafts[uid] || blank;
  const setDraft = (uid, patch) => setDrafts({ ...drafts, [uid]: { ...draftFor(uid), ...patch } });
  const saveDriver = async (m) => {
    const f = draftFor(m.userId);
    setBusy(true); setMsg(null);
    const r = await call({
      action: "save_driver", userId: m.userId, isDriver: f.isDriver,
  ...
  const saveTerritory = async () => {
    ...
    setBusy(true); setMsg(null);
    const r = await call({ action: "save_territory", id: tDraft.id, name: tDraft.name, description: tDraft.description }, "Territory saved.");
  ...
  return (
    <div style={S.card}>
      <CardHead title="Drivers & delivery territories" count={null}
      ...
      <p style={{ fontSize: 12, color: "#64748B", fontWeight: 600, lineHeight: 1.5, margin: "0 0 12px" }}>
        Drivers power the <strong>Delivery Schedule</strong>: each driver's deck length sets how much fits on their loads,
        max width caps which buildings they can haul, and their territory routes the right deliveries to the right driver.
      </p>
      {msg && msg.ok && <div style={S.okMsg}>{msg.ok}</div>}
      {msg && msg.err && <div style={S.err}>{msg.err}</div>}
      ...
      {data && (<>
        ...
        {territories.map((t) => (
          tDraft && tDraft.id === t.id ? (
            ...
            <div key={t.id} style={{ display: "flex", gap: 10, alignItems: "center", border: "1px solid #E2E8F0", borderRadius: 10, padding: "8px 11px", marginBottom: 7, opacity: t.active === false ? 0.55 : 1 }}>
              ...
              <button type="button" style={{ ...S.btn("#F1F5F9", "#475569"), padding: "4px 9px", fontSize: 11 }} onClick={() => setTDraft({ id: t.id, name: t.name, description: t.description || "" })}>Edit</button>
        ...
        {/* ── Drivers ── */}
        <div style={{ ...S.lbl, margin: "16px 0 8px" }}>Drivers</div>
        {team.length === 0 && <p style={{ fontSize: 12.5, color: "#64748B" }}>No team members yet — invite people in Team &amp; commissions below.</p>}
        {team.map((m) => {
          const f = draftFor(m.userId);
          return (
            <div key={m.userId} style={{ border: "1px solid #E2E8F0", borderRadius: 10, padding: "9px 12px", marginBottom: 8, background: f.isDriver ? "#FBFCFE" : "#FFF" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
                <strong style={{ fontSize: 13 }}>{m.name || m.userId.slice(0, 8)}</strong>
                <span style={schedChip("#F1F5F9", "#475569")}>{m.role === "owner" ? "Owner" : m.role === "admin" ? "Admin" : "Team member"}</span>
                <label style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 12.5, fontWeight: 700, color: "#334155", cursor: "pointer", marginLeft: "auto" }}>
                  <input type="checkbox" checked={f.isDriver} onChange={(e) => setDraft(m.userId, { isDriver: e.target.checked })} />
                  Driver
                </label>
              </div>
              {f.isDriver && (<>
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(130px, 1fr))", gap: "8px 10px", marginTop: 9 }}>
                  <div><span style={lbl}>Truck / trailer</span><input style={inp} value={f.truckName} onChange={(e) => setDraft(m.userId, { truckName: e.target.value })} placeholder="40' gooseneck" /></div>
                  <div><span style={lbl}>Deck length (ft)</span><input style={inp} value={f.deck} onChange={(e) => setDraft(m.userId, { deck: e.target.value })} placeholder="40" /></div>
                  <div><span style={lbl}>Max building width (ft)</span><input style={inp} value={f.maxW} onChange={(e) => setDraft(m.userId, { maxW: e.target.value })} placeholder="14" /></div>
                  <div><span style={lbl}>Wide-load capable</span>
                    <select style={inp} value={f.wide ? "yes" : "no"} onChange={(e) => setDraft(m.userId, { wide: e.target.value === "yes" })}>
                  ...
                  <div style={{ marginTop: 8 }}>
                    ...
                    <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginTop: 3 }}>
                      {activeTerritories.map((t) => {
                        ...
                        return (
                          <button key={t.id} type="button" onClick={() => toggleTerr(m.userId, t.id)} title={t.description || t.name}
              ...
              <div style={{ display: "flex", marginTop: f.isDriver ? 9 : 0 }}>
                {(f.isDriver || (data.drivers || []).some((p) => p.user_id === m.userId && p.is_driver)) && (
                  <button type="button" disabled={busy} style={{ ...S.btn(ACCENT, "#FFF"), padding: "6px 13px", fontSize: 12, marginLeft: "auto", marginTop: f.isDriver ? 0 : 8 }} onClick={() => saveDriver(m)}>Save driver</button>
                )}
              </div>
            </div>
          );
...
function LocationsCard() {
  ...
  return (
    <div style={S.card}>
      ...
      <div style={{ fontSize: 12.5, color: "#64748B", marginBottom: 12, lineHeight: 1.5 }}>
        Every inventory building — and, once Orders ships, every customer order — takes the next number
        in one shared sequence, in the order they're created.
      </div>
      ...
      <div style={{ background: "#EFF6FF", border: "1px solid #BFDBFE", color: "#1E3A8A", borderRadius: 10, padding: "10px 14px", fontSize: 12.5, fontWeight: 600, marginTop: 14, lineHeight: 1.5 }}>
        📍 Assigning team members to locations arrives with Build &amp; Delivery Scheduling — the location list you build here is what it will use.
      </div>
    </div>
  );
}
...
function DesignerTab({ clientId, onSaved, openDesign = null, setup3d = null }) {
  ...
  return (
    <div style={{ maxWidth: 1080, margin: "0 auto", width: "100%" }}>
      <SS clientId={clientId} embedded onSaved={onSaved} openDesign={openDesign} setup3d={setup3d} />
    </div>
  );
}
                              ...
                              {resetInfo[u.userId].error ? (
                                ...
                                <div style={{ background: "#F8FAFC", border: "1px solid #E2E8F0", borderRadius: 8, padding: "8px 10px" }}>
                                  ...
                                  <div style={{ fontSize: 11, color: "#94A3B8", marginTop: 5 }}>
                                    The link opens structurestudio.app/portal — a password set there works everywhere, including beta.
                                  </div>
                                </div>
                              )}
                            </td>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(220px,1fr))", gap: 12 }}>
            ...
            <div>
              ...
              <div style={{ fontSize: 11, color: idErr ? "#DC2626" : "#94A3B8", marginTop: 6 }}>
                {idErr || (idTouched ? "Becomes junior-barns.structurestudio.app." : "Auto-generated from the company name — edit to override.")}
              </div>
            </div>
            <div>
  ...
  return (
    <div style={{ ...S.card, maxWidth: 720 }}>
      ...
      <div style={{ fontSize: 11.5, color: "#94A3B8", marginTop: 12, lineHeight: 1.5 }}>
        Per-person rates and the Commissions report come next — this sets the rules they'll run on. Every period is still reviewed and approved by you before it's payable.
      </div>
    </div>
  );
}
...
function CommissionTeam() {
  const [data, setData] = useState(null);       // { members, canSeeRates, isOwner, role } | { members: [] }
  const [err, setErr] = useState(null);
  const [busy, setBusy] = useState(false);
  const [msg, setMsg] = useState(null);         // { ok } | { err }
  const [add, setAdd] = useState({ fullName: "", email: "", role: "user" });
  ...
  const addUser = async () => {
    ...
    try {
      const r = await call({ action: "add_user", email: add.email.trim(), fullName: add.fullName.trim(), role: add.role });
      ...
      setAdd({ fullName: "", email: "", role: "user" });
      await load();
    } catch (e) { setMsg({ err: e.message }); }
    setBusy(false);
  };
  ...
  const roleChip = (r) => { const c = { owner: ["#EDE9FE", "#5B21B6"], admin: ["#DBEAFE", "#1E40AF"], user: ["#F1F5F9", "#475569"] }[r] || ["#F1F5F9", "#475569"]; return <span style={{ background: c[0], color: c[1], borderRadius: 20, padding: "3px 10px", fontSize: 11, fontWeight: 700, textTransform: "capitalize" }}>{r}</span>; };
  ...
  return (
    <div style={{ ...S.card, maxWidth: 860 }}>
      ...
      <div style={{ ...S.h2, marginBottom: 3 }}>Team &amp; commission rates</div>
      <p style={{ fontSize: 12.5, color: "#64748B", lineHeight: 1.5, margin: "0 0 16px" }}>
        {isOwner
          ? "Add the people who work in your account and set each one's commission rate. Rates and other people's payouts are visible to you only — grant an admin access per person with the toggles."
          : "Add teammates for your account. Commission rates are set by the owner and shown as ••• here."}
      </p>
      ...
      <div style={{ display: "flex", gap: 10, flexWrap: "wrap", alignItems: "flex-end", background: "#F8FAFC", border: "1px solid #E2E8F0", borderRadius: 10, padding: 14, marginBottom: 16 }}>
        <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
          ...
          <input value={add.fullName} onChange={(e) => setAdd((p) => ({ ...p, fullName: e.target.value }))} placeholder="Jordan Blake" style={{ ...S.input, width: 150 }} />
        </div>
        <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
          <span style={S.lbl}>Email</span>
          <input value={add.email} onChange={(e) => setAdd((p) => ({ ...p, email: e.target.value }))} placeholder="jordan@company.com" style={{ ...S.input, width: 200 }} />
        </div>
        <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
          <span style={S.lbl}>Role</span>
          <select value={add.role} onChange={(e) => setAdd((p) => ({ ...p, role: e.target.value }))} style={{ ...S.input, width: 120 }}>
      ...
      {invite && (
        <div style={{ background: "#EEF2FF", border: "1px solid #C7D2FE", borderRadius: 9, padding: "11px 13px", marginBottom: 14, fontSize: 12.5, color: "#3730A3" }}>
          <b>{invite.email}</b> {invite.emailSent ? "was emailed a setup link." : "was added."} Share this one-time set-password link if the email doesn't arrive:
      ...
      <div style={{ overflowX: "auto" }}>
        <table style={{ width: "100%", borderCollapse: "collapse" }}>
          <thead><tr>
            <th style={th}>Name</th><th style={th}>Email</th><th style={th}>Role</th><th style={th}>Commission&nbsp;%</th>
          ...
          <tbody>
            {members.map((m) => (
              <tr key={m.userId}>
                <td style={{ ...td, fontWeight: 700 }}>{m.fullName || <span style={{ color: "#94A3B8" }}>—</span>}{m.isSelf && <span style={{ fontSize: 11, color: "#94A3B8", fontWeight: 500 }}> (you)</span>}</td>
                <td style={td}>{m.email}</td>
                <td style={td}>{roleChip(m.role)}</td>
                <td style={td}>
                  {m.rateHidden
                    ? <span style={{ color: "#94A3B8", fontWeight: 800, letterSpacing: 2 }}>🔒 •••</span>
                    : m.role === "owner"
                      ...
                      : <span style={{ display: "inline-flex", alignItems: "center", gap: 3 }}>
                          <input
                            ...
                            style={{ ...S.input, width: 64, textAlign: "right", padding: "6px 8px" }} />
                          <span style={{ color: "#94A3B8" }}>%</span>
                        </span>}
                </td>
                {isOwner && <td style={td}>{m.role === "owner" ? <span style={{ fontSize: 11, color: "#94A3B8" }}>Always</span> : <span onClick={() => !busy && setGrant(m, "seesAllPayouts", !m.seesAllPayouts)} style={{ cursor: "pointer" }} title="Let this person see everyone's payouts">{sw(!!m.seesAllPayouts)}</span>}</td>}
                {isOwner && <td style={td}>{m.role === "owner" ? <span style={{ fontSize: 11, color: "#94A3B8" }}>Always</span> : <span onClick={() => !busy && setGrant(m, "fullAccess", !m.fullAccess)} style={{ cursor: "pointer" }} title="Let this person see and edit commission rates">{sw(!!m.fullAccess)}</span>}</td>}
                <td style={{ ...td, whiteSpace: "nowrap", textAlign: "right" }}>
                  {m.isSelf || m.role === "owner" ? null : removing && removing.userId === m.userId ? (
                    ...
                    <button onClick={() => setRemoving(m)} disabled={busy} style={{ background: "none", border: "none", color: "#94A3B8", cursor: "pointer", fontSize: 12, fontWeight: 700, fontFamily: "inherit" }}>Remove</button>
                  )}
                </td>
              </tr>
            ))}
            {members.length === 0 && <tr><td colSpan={isOwner ? 7 : 5} style={{ ...td, color: "#64748B" }}>No teammates yet — add someone above.</td></tr>}
          </tbody>
        </table>
      </div>
...
function CommissionsReport() {
  ...
  const openSplit = (e) => setSplitFor({ entry: e, rows: [{ userId: e.earnerUserId || "", share: "50" }, { userId: "", share: "50" }] });
  const saveSplit = () => {
    ...
    if (splits.length < 2) { setMsg({ err: "Pick at least two people for a split." }); return; }
    if (Math.abs(sum - 100) > 0.01) { setMsg({ err: `Shares must add up to 100% (currently ${sum}%).` }); return; }
    setSplitFor(null);
    act({ action: "split_entry", entryId: splitFor.entry.id, splits }, "Split saved.");
  };
                      ...
                      {canSeeRates && (
                        <td style={{ ...td, whiteSpace: "nowrap" }}>
                          ...
                          {e.kind === "clawback" ? <span style={{ color: "#94A3B8", fontSize: 11 }}>—</span>
                            ...
                            : e.status === "excluded" ? <button onClick={() => act({ action: "set_excluded", entryId: e.id, excluded: false })} disabled={busy} style={actLink("#3D3672")}>Restore</button>
                            : (
      ...
      {splitFor && (
        <div onClick={(ev) => { if (ev.target === ev.currentTarget) setSplitFor(null); }}
          ...
          <div style={{ background: "#FFF", borderRadius: 14, maxWidth: 460, width: "100%", boxShadow: "0 24px 60px rgba(0,0,0,0.3)", overflow: "hidden" }}>
            <div style={{ background: ACCENT, color: "#FFF", padding: "15px 18px", fontSize: 15.5, fontWeight: 800 }}>Split order #{splitFor.entry.orderNo}</div>
            <div style={{ padding: "16px 18px" }}>
              <p style={{ fontSize: 12.5, color: "#64748B", margin: "0 0 12px" }}>Split this sale's credit between reps — each earns their own rate on their share. Shares must total 100%.</p>
              {splitFor.rows.map((r, i) => (
                <div key={i} style={{ display: "flex", gap: 8, alignItems: "center", marginBottom: 8 }}>
                  ...
                  <span style={{ color: "#94A3B8" }}>%</span>
                  {splitFor.rows.length > 2 && <button onClick={() => setSplitFor((s) => ({ ...s, rows: s.rows.filter((_, j) => j !== i) }))} style={{ background: "none", border: "none", color: "#94A3B8", cursor: "pointer", fontWeight: 700, fontFamily: "inherit" }}>✕</button>}
                </div>
              ))}
              ...
              <div style={{ display: "flex", gap: 8, marginTop: 15 }}>
                <button onClick={saveSplit} disabled={busy} style={{ ...S.btn(ACCENT, "#FFF"), flex: 1 }}>Save split</button>
...
function SettingsShell({ clientId, viewingLabel = null, sub: subProp = null, onSub = null, isOwner = false, isAdmin = false, schedUnlocked = false }) {
  ...
  const TABS = [
    ...
    ...(isAdmin ? [["team", "Team", "People, roles, and commission rates"]] : []),
  ...
  return (
    <div>
      ...
      {sub === "structures" && <PricingCsv viewingLabel={viewingLabel} />}
      ...
      {sub === "quickbooks" && <QuickBooksView clientId={clientId} viewingLabel={viewingLabel} />}
      ...
      {sub === "team" && (<><LocationsCard />{schedUnlocked && <DriversTerritoriesCard />}<CommissionTeam /></>)}
...
function Dashboard({ session }) {
  ...
  // that array is the role escape hatch and would hand the operator console to every team
  // member. Content renders are ALSO gated (and the server re-checks regardless).
  const activeTab = ssClampTab(tab, isOperator, canAdmin);
  ...
  const schedUnlocked = isOperator || (!viewing && !!entitlement && !!(entitlement.features && entitlement.features.schedule_builds));
    ...
    <div className="ss-shell">
      <aside className="ss-side">
        ...
        <nav className="ss-nav">
          ...
          {soonItem("orders", "Orders", "In Dev")}
          {soonItem("commissions", "Commissions", "In Dev")}
          ...
          {canAdmin && navItem("quickbooks", "QuickBooks", "In Dev")}
        </nav>

        <div className="ss-navlabel">Coming Soon</div>
        <nav className="ss-nav">
          {soonItem("on-demand-pricing", "RealTime Pricing", "3rd Qtr")}
          {schedUnlocked ? navItem("repairs", "Repairs") : soonItem("repairs", "Repairs", "New")}
          {/* No longer "3rd Qtr": 3D renders today, and this tab now reports which of the
              tenant's styles have a 3D look set rather than advertising the feature. */}
          {navItem("view-3d", "3D Design", "New")}
          {soonItem("rent-to-own-contracts", "Rent to Own", "4th Qtr")}
          {soonItem("reports", "Reports", "4th Qtr")}
          {/* Live for operators + subscribed tenants; a "New" badge invites everyone else. */}
          {schedUnlocked ? navItem("build-schedule", "Build Schedule") : soonItem("build-schedule", "Build Schedule", "New")}
          {schedUnlocked ? navItem("delivery-schedule", "Delivery Schedule") : soonItem("delivery-schedule", "Delivery Schedule", "New")}
        ...
        {/* Settings pinned at the bottom, just above the footer divider */}
        {canAdmin && (
          ...
          <div className="ss-inner">
            ...
            {!gateLocked && activeTab === "inventory" && (
              <InventoryTable key={"inv-" + effClientId} clientId={effClientId} isAdmin={canAdmin}
                ...
                onOpenDesign={openInDesigner}
                onSendEstimate={(u) => openInDesigner(u.shortCode, null, { asNew: true, inventoryUnitId: u.id, unitSerial: u.serial })}

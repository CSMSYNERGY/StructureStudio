...
//
// ⛔ MIGRATION 242 BEFORE THE DEPLOY. Every charge passes `p_meter_kind`, which only 242's
...
// The other order is safe: 242 defaults the new parameter, so callers that omit it keep working.
//
 ...
 *   `lookupId` the `tax_lookups` row id (migration 242). The deliberate paths — the verify
 ...
 *
 * The meter kind is recorded on the wallet row (`p_meter_kind`, migration 242), which is what

-- 081_get_config_exclude_archived_inclusions: an ARCHIVED option must stop counting as
-- "included" in any building. Archiving retires an option from new builds; leaving it in a
-- building's base-price inclusions would keep silently netting it on quotes — and since the
-- pricing CSV no longer shows an archived option as a column, there'd be no way to see or clear
-- that stale inclusion. So get_config now DROPS any building_size_inclusions row whose item_key
-- points at an archived option (built-in client_layout_items.archived OR catalog
-- fixture_items.archived) when it assembles sizeInclusions / sizeInclusionQty.
--
-- Read-time filter (not a delete): it fixes ALREADY-archived options immediately, applies to
-- both the designer's live pricing and the estimate (submit-estimate filters the same way), and
-- REVERSES automatically if the option is un-archived. The inclusion row itself is untouched.
-- HAND-APPLY via MCP; record in ledger. Only the two inclusion subqueries changed vs 075.

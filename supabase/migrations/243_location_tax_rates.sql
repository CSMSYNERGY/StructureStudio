-- 243_location_tax_rates.sql — a tax rate per sales location, and the location a quote was sold
-- from.
--
-- APPLY BY HAND (`supabase db query --linked` with the SQL INLINE from Git Bash, never --file;
-- or the SQL editor), as the owner, then record version 243 in
...
-- --linked "<newline>$(cat 243_location_tax_rates.sql)". The CLI prints no notices, so exit 0
...
-- below does not hold. Independent of 242; either may be applied first.
--
-- ── WHY ───────────────────────────────────────────────────────────────────────────────────
...
comment on column public.builder_locations.tax_rate is
  'Migration 243. This lot''s local sales tax rate as a FRACTION (0.0725 = 7.25%). NULL = none; quotes from this lot use the company rate. Set in settings (save_location_tax). A local rate, not a verified one.';
comment on column public.builder_locations.tax_label is
  'Migration 243. What the tax row is called on a quote taxed at this lot''s rate. NULL = the company label.';
...
begin
  begin
    ...
    values ('probe-243-tenant-a', 'Probe lot A', 'MO', '63090', 0.0725, 'Sales tax')
    returning id into a_loc;
    insert into public.builder_locations (client_id, name)
    values ('probe-243-tenant-b', 'Probe lot B')
    returning id into b_loc;

    insert into public.designs (short_code, client_id, bldg_w, bldg_h, status, sales_location_id)
    values ('SS-PROBE243QA', 'probe-243-tenant-a', 10, 12, 'draft', a_loc);

    -- Another tenant's location is refused by the key, not by a caller remembering to check.
    begin
      update public.designs set sales_location_id = b_loc where short_code = 'SS-PROBE243QA';
      raise exception '243 probe: a design was pointed at another tenant''s location';
    ...
    begin
      ...
      raise exception '243 probe: a percent-shaped location rate was stored';
    exception when check_violation then null;
    end;
    begin
      update public.builder_locations set tax_label = repeat('x', 41) where id = a_loc;
      raise exception '243 probe: a 41-character tax label was stored';
    exception when check_violation then null;
    end;
    ...
    select client_id, sales_location_id into got_client, got_loc
      from public.designs where short_code = 'SS-PROBE243QA';
    if got_client is distinct from 'probe-243-tenant-a' or got_loc is not null then
      raise exception '243 probe: deleting a location did not null only sales_location_id (client_id %, location %)',
        got_client, got_loc;
    end if;
  ...
  exception
    when others then
      if sqlerrm = 'ROLLBACK_PROBE' then
        raise notice '243: location rates are bounded; a design can only name its own tenant''s location; deleting a location nulls only the link';
      else
        raise;
      end if;
  end;

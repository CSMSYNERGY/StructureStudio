-- 118_get_fixtures_colors: fixtures carry their color behavior; the response carries
-- the client's window-color list.
--
-- Body regenerated from the LIVE pg_get_functiondef on 2026-08-24 (same wholesale-
-- rewrite hazard as get_config — never splice into the repo file). Changes:
--   * each item gains colorMode / hasTrimColor via the internalOnly ||-merge (they are
--     not prices, so they ride outside the priced/unpriced branches), plus a fixedColor
--     {id,label,hex} object when color_mode='fixed' points at a palette row
--   * the return object gains windowColors[] — id/label/hex/isDefault always, rate
--     only when show_pricing (flat $ per window; the anon designer of a hidden-pricing

-- 036_get_config_layout_pricing_full: give the designer everything it needs to display each
-- placed layout item's price EXACTLY as submit-estimate will charge it. Migration 035 exposed
-- only { item_key: rate } (flat, style_id IS NULL, no method), which is enough for the flat
-- rough-opening price but NOT for the 7 pricing methods the estimate actually applies.
--
-- Adds two show_pricing-gated fields to get_config's output:
--   layoutPricing — { item_key: { rate, method, byStyle: { <styleKey>: { rate, method } } } }
--                   `method` is the layout_item_pricing.pricing_method (each / lineal_ft /
--                   sqft_option / sqft_building / perimeter_building / pct_building_price /
--                   pct_estimate_total). `byStyle` carries per-building-style overrides keyed by
--                   building_styles.key so the browser can resolve style override → NULL default,
--                   matching submit-estimate's layoutRates precedence.
--   sizePricing   — { <styleKey>: { <size label>: { basePrice, widthFt, lengthFt } } } for active
--                   sizes. Lets the designer compute buildingArea / buildingPerimeter / building
--                   base price for the sqft_building, perimeter_building, pct_building_price and
--                   pct_estimate_total methods, using the SAME numbers submit-estimate reads.
--
-- The pre-existing flat `layoutPrices` map is kept for backward compatibility (the rough-opening
-- price cell still reads C.layoutPrices.roughOpening). All three price fields are empty ({}) when
-- the tenant's client_settings.show_pricing is off, so prices never leak to anon for hidden tenants.

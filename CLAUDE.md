...
- **v2 (migration 160):** steps carry `section` (display grouping — a header renders whenever it changes walking the position order; there is no second ordering model) and `image_url` (a screenshot). `image_url` may hold ONLY a URL in the public `setup-screens` bucket — `setup_upload_image` mints the path, and both save actions refuse anything else, because the column lands in an `<img src>` in every tenant's browser. The bucket is public with no storage policies on purpose (the `branding`/`fixtures` shape): operator-authored screenshots of the product's own screens, identical for every tenant. Any new column must be added to BOTH copy sites (`admin-catalog` create_client and `setup_assign_template`) and to the two explicit select lists (`SetupChecklist`'s browser read and `portal-setup` `list`).
- ⚠️ **KEEP THE CHECKLIST CURRENT (Carolyn 2026-08-28).** Shipping any change to a Settings surface or a tenant-facing setup flow INCLUDES reviewing Projects → Client Setup in the same session: update the affected step's wording and re-capture its screenshot (view-as a demo tenant, never a screen showing real customer data). The template is DATA — update it through the editor or `setup_template_save`, never a migration; migration 160's seed is the historical record of the v2 content, not the live truth. Existing tenants keep their copies by design; refresh an individual client's list by hand only where it matters.
...
- **Internal accounts use the checkbox, not the flag.** The admin console's New client form has **Non-billable — CSM Synergy internal / demo / testing**, off by default, which has `admin-catalog` create the `client_settings` row with `billing_exempt = true`. Use it for demos and test tenants instead of hand-editing the database. `internal_account` (migration 169) is a separate column with no UI, set only on `structure-studio`. It permits a sending domain on one of our own apexes and admits that tenant's team to CSM's internal Projects console (`_shared/projectsAccess.ts`, door 2), and since 2026-09-24 it also changes Billing, below. ⚠️ **Never set it on any tenant but our own** — it is not a demo-view switch; on a customer or demo tenant it would open CSM's roadmap to their owner.
- **Our own account shows the PROSPECT view on Billing, and cannot buy.** Carolyn demos from `structure-studio`, so its Subscription tab shows the Founding banner, real prices and selectable tiles, not the comped view other Non-billable accounts get (`demoView` in BillingView, keyed on `entitlement.reason === "internal"`). It has a **real card on file**, so checkout is stopped twice: `subscribe()` returns at its first line with a "demo account" note, and portal-billing's `subscribe` answers 409 for an internal account ahead of the vault and the gateway. ⚠️ **The Wallet tab is NOT guarded** — Add funds and auto top-up still charge that card for real (Ahsan's call, 2026-09-24).
- **Founding pricing is yearly-only — `FOUNDING_ANNUAL_ONLY` in `portal/03-catalog.jsx` is the one switch** (Carolyn 2026-09-24). The Monthly button stays on each tile, dimmed with a hover hint, and does nothing; the transition banner quotes only /yr. ⚠️ Browser-only: the `_monthly` plan rows and portal-billing's `subscribe` still sell monthly to a direct request. That was deliberate while this lived on beta alone (the live frontend still offered Monthly); add the server refusal when it is promoted. Set the constant to `false` to reopen monthly.

...
The cutover is **done**. The multi-tenant frontend (config via `get_config`, designs via `load_design`/`save_design`, PDFs under `{client_id}/`) is deployed to **production** (`structurestudio.app`), and `005_cutover.sql` + `015_config_rls_scope.sql` are applied. **Deploy source:** the **`beta` branch is the production line** — `beta.structurestudio.app` ships from it, and per the project owner all work stays on `beta` and is **never** merged to `main`. ⚠️ `main` is a **frozen legacy single-tenant build on a throwaway Supabase DB** (per its tip commit, 2026-06-15) with no portal/admin/migrations — it is **not** the production source and must never be used as a merge target. Verified live: **every public table has RLS on and zero anon-readable policies** — anon reaches data only through the capability RPCs (`get_config`/`get_catalog`/`load_design`); `designs_anon_all` and `floor_plans_public_all` are dropped; `client_configs` is owner-scoped. Storage writes require the `{client_id}/SS-….pdf` shape (no anon delete, no bucket listing). Pre-cutover floor-plan files remain at the bucket root (still publicly readable); re-pathing them under `{client_id}/` via COPY is optional cleanup (see `CUTOVER_HANDOFF.md` Task 3).

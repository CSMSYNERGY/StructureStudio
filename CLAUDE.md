...
- **PRODUCTION (canonical) is on Cloudflare Workers** at **app.structurestudiosuite.com** — zone route → worker `structurestudio-app`, serving `main` (byte-parity verified 2026-08-06). Workers Builds is connected to `main`, but its first git build did not deploy (under diagnosis — check the worker's build history in the dashboard); until that's confirmed working, treat a `main` change as shipped only after verifying, and use the manual fallback if needed: a detached LF worktree of `origin/main` + `npx wrangler deploy` (CRLF checkouts break byte-parity — see the migration work log). **LEGACY: structurestudio.app is still Netlify** (old customer links; rebrand to Structure Studio Suite over a trademark conflict) pending its sunset redirect. The bare apex structurestudiosuite.com has NO DNS record — only `app` and `beta` exist. The old Netlify beta (beta.structurestudio.app) also still serves — and it is NOT stale: verified 2026-08-06 it carries the current beta build (`/vendor/` tags, `?v=20260806a`), i.e. **Netlify still auto-deploys every `beta` push** in parallel with the Cloudflare worker. Still don't verify against it (the worker is canonical), but know that Googlebot actively crawls it and gets live beta content — an earlier version of this sentence called it a stale copy, which was wrong.

...
export default function StructureStudio({ config = DEFAULT_CONFIG }) {
  ...
  const renderExportCanvas = () => {
    ...
    items.forEach((item) => {
      ...
      else if (item.type === "ramp") { ctx.fillText("RAMP", 0, (item.wall === "south" || item.wall === "west") ? (ih / 2 + 11) : -(ih / 2 + 3)); }
              ...
              <g key={item.id} transform={`translate(${item.x},${item.y}) rotate(${item.rotation})`}
                ...
                {item.type === "loft" ? (
                  ...
                  <>
                    ...
                    {item.type === "ramp" ? (
                      <text x={0} y={(item.wall === "south" || item.wall === "west") ? (ih / 2 + 10) : -(ih / 2 + 3)} textAnchor="middle" fill={cfg.color} fontSize={9} fontWeight="700">RAMP</text>
                    ) : isWB ? (
                      <>

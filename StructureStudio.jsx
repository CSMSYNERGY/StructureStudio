...
// Lazy-load Google Maps JS API with the Places library. Idempotent: returns the
// same promise on subsequent calls. Resolves to window.google. Rejects if no key.
...
export default function StructureStudio({ config = DEFAULT_CONFIG }) {
  ...
  // Google Maps Places autocomplete on the street-address input. When
  // C.googleMapsApiKey is set, we lazy-load Maps JS and attach an Autocomplete
  // widget; on selection we parse address_components and populate street/city/
  // state/zip. When the key is empty, this is a no-op and the input behaves as
  // a plain text field.
  const attachStreetAutocomplete = useCallback((node) => {
    ...
    loadGoogleMapsPlaces(C.googleMapsApiKey).then((google) => {
      ...
      ac.addListener("place_changed", () => {
        ...
        const state = get("administrative_area_level_1"); // full state name to match <select> options
        const zip = (get("postal_code") || "").replace(/\D/g, "").slice(0, 5);
        setContact((p) => ({
          ...p,
          ...(street ? { street } : {}),
          ...(city ? { city } : {}),
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: 10 }}>
              {C.contactFields.includes("street") && (
                <div style={{ flex: "2 1 200px" }}>
                  ...
                  <input type="text" ref={attachStreetAutocomplete} autoComplete="street-address" value={contact.street} onChange={(e) => setContact((p) => ({ ...p, street: e.target.value }))} placeholder="123 Main St" style={{ ...S.sel, width: "100%", boxSizing: "border-box" }} />
                </div>
              )}

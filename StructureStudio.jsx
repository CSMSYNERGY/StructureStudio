...
//   2. `?client=<id>` URL param picks which row to fetch. Default: DEFAULT_CLIENT_ID
//      (this Netlify site's tenant).
//   3. On fetch failure (network, missing row) we render an error screen with a
...
export default function StructureStudio({ config: configProp = null }) {
  ...
  useEffect(() => {
    ...
    const clientId = new URLSearchParams(window.location.search).get("client") || DEFAULT_CLIENT_ID;
    let cancelled = false;
    (async () => {
      try {
        ...
        setState({ status: "error", clientId, message: e?.message || "Network error." });
      }
    })();
    return () => { cancelled = true; };
  }, [state.status]);

...
function Structure3DViewer({ bldgW, bldgH, items, itemTypes, styleValue, painted, paintBody, paintTrim, frontWall, scale, mgX, mgY, accent, style3d, roofType, roofColorHex, paintEnabled, onPaintChange, onWallHeight, onItemAdd, onItemMove, onItemSelect, onSnapshot, onClose }) {
  ...
  useEffect(() => {
    ...
    loadThree().then((bundle) => {
      ...
      const FRONT_AZ = { south: 0, north: Math.PI, east: Math.PI / 2, west: -Math.PI / 2 }[fw] || 0;
      const setViewPreset = (relDeg, polDeg) => {
        const e = engineRef.current;
        if (!e) return;
        const t = e.controls.target;
        const r = e.camera.position.distanceTo(t) || dist;
        const az = FRONT_AZ + (relDeg * Math.PI) / 180;
      ...
      engineRef.current = { renderer, scene, camera, controls, model, sky, render, resize, ro, applyShellMode, setViewPreset, disposeInteraction, setLiveColors, setWallHeight, interior: false, roofOn: true, envOn: true };
      resize();
      setPhase("ready");
    }).catch((err) => {

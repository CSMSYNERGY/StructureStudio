...
function StructureStudioInner({ config }) {
  ...
  const renderOption = (opt) => {
    ...
    if (opt.type === "counter") {
      ...
      // Paint has no No-Paint/Painted pills — the Body + Trim selects are always shown and
      // each carries an "Unpainted" choice. sel.paint ("No Paint"/"Painted") is still the
      // source of truth (save/load/submit contract); it's derived from these selects here.
      // "Both must match": picking Unpainted on either side unpaints the whole building;
      // picking a real color on either side paints it (and clears the other side's Unpainted).
      const paintField = (kind) => {
        const baseColors = palette.filter((c) => (kind === "body" ? c.siding : c.trim));
        const val = paintColors[kind] || "";
        const set = (v) => setPaintColors((p) => ({ ...p, [kind]: v }));
        const labelTxt = kind === "body" ? "Body:" : "Trim:";
        const unpainted = sel[opt.id] === "No Paint";
        // No palette configured for this side → free-text. Empty = unpainted, any text = painted.
        if (baseColors.length === 0) {
          return (
            <label style={PAINT_LBL}>{labelTxt}
              <input type="text" value={unpainted ? "" : val}
                onChange={(e) => { const v = e.target.value; setSel((p) => ({ ...p, [opt.id]: v ? "Painted" : "No Paint" })); set(v); }}
                placeholder="Unpainted / enter color" style={PAINT_INPUT} />
            </label>
          );
        }
        // Prepend a synthetic "Unpainted" row (no swatch) as the top dropdown choice.
        const colors = [{ id: "__unpainted__", label: "Unpainted", hex: null }, ...baseColors];
        const match = baseColors.find((c) => c.label === val && !c.allowCustom);
        const customColor = baseColors.find((c) => c.allowCustom);
        const isCustom = !unpainted && (paintCustom[kind] || (!match && !!val && !!customColor));
        const selectVal = unpainted ? "Unpainted" : (isCustom && customColor ? customColor.label : (match ? match.label : ""));
        const onSel = (label) => {
          if (label === "Unpainted") {
            setSel((p) => ({ ...p, [opt.id]: "No Paint" }));
            setPaintColors({ body: "", trim: "" }); setPaintCustom({ body: false, trim: false });
            return;
          }
          setSel((p) => ({ ...p, [opt.id]: "Painted" }));
          const c = baseColors.find((x) => x.label === label);

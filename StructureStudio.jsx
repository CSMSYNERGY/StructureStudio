...
// (read-only via RLS); the service-role key never leaves the Edge Functions.
...
// ─── Default tenant ───
// When no ?client= URL param is present, the loader fetches this client's config
// from public.client_configs. The Junior Barns row in that table is the source
// of truth for its branding, building styles, options, etc. — there is no in-source
// copy. To change Junior Barns, edit the row, not this file.
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: 10 }}>
              ...
              {C.contactFields.includes("state") && (
                <div style={{ flex: "1 1 160px" }}>
                  ...
                  <select value={contact.state} onChange={(e) => setContact((p) => ({ ...p, state: e.target.value }))} style={{ ...S.sel, width: "100%", boxSizing: "border-box" }}>
...
// ─── Default export: client-config loader ───
// Every page load fetches the config from public.client_configs — there is no
// in-source copy of any client. Resolution order:
...
//   3. Subdomain — `juniorbarns.structurestudio.app` → "juniorbarns". Skipped
//      for the apex, IPs, localhost, .pages.dev project roots, and the reserved
//      "beta" subdomain (which is the beta-deploy alias, not a tenant).
//   4. `?id=<short_code>` share-link — look up the owning tenant from the
//      `designs` row so a rep clicking someone else's link gets the right config.
//   5. Fallback: DEFAULT_CLIENT_ID.
//   On fetch failure (network, missing row) we render an error screen with a
//   retry button rather than silently falling back to a wrong-tenant default.
export default function StructureStudio({ config: configProp = null }) {
  ...
  useEffect(() => {
    ...
    // Tenant path: only derive a client_id from <sub>.structurestudio.app.
    // Anything else (apex, *.netlify.app deploy/preview hosts, localhost, IPs,
    // env labels) falls back to DEFAULT_CLIENT_ID — so a deploy hostname is
    ...
    (async () => {
      try {
        ...
        // Share-link path: ?id=<short_code> without ?client= or subdomain means
        // a rep opened someone else's saved design. The design row records which
        // tenant owns it; we look that up so the right config wraps the inner load.
        if (!clientId && designShortCode) {
          const { data: design, error: dErr } = await sb
            .from("designs")
            .select("client_id")
            .eq("short_code", designShortCode)
            .single();
          if (cancelled) return;
          if (dErr || !design?.client_id) {
        ...
        if (error || !data?.config) {
          ...
          setState({ status: "error", clientId, message: error?.message || "Configuration not found." });
        } else {
          setState({ status: "ready", config: data.config });
        }
      } catch (e) {
        if (cancelled) return;
        console.warn("Client config fetch error:", e);
        setState({ status: "error", clientId: clientId || DEFAULT_CLIENT_ID, message: e?.message || "Network error." });
      }
    })();
    return () => { cancelled = true; };
  }, [state.status]);

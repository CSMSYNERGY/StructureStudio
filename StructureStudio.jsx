...
function buildShed3DModel(THREE, p) {
  ...
  // Ground + floor slab
  const ground = new THREE.Mesh(new THREE.CircleGeometry(Math.max(bldgW, bldgH) * 2.4, 48), mat(D3_COLORS.ground));
  ...
  // Opening vertical extent by item type (plan §4.4 defaults).
  const openingSpan = (it) => {
    if (it.type === "window") return [D3.WINDOW_SILL, D3.WINDOW_SILL + D3.WINDOW_H];
    if (it.type === "roughOpening") return [0, D3.RO_H];
    return [0, D3.DOOR_H]; // singleDoor / doubleDoor
  };
  ...
  Object.keys(WALLS).forEach((wname) => {
    ...
    // strips below/above each opening range.
    let cursor = 0;
    ranges.forEach((rg) => {
      if (rg.a0 > cursor + 0.01) wallsGroup.add(wallBox(wallMat, wf, cursor, rg.a0, 0, H));
      if (rg.y0 > 0.01) wallsGroup.add(wallBox(wallMat, wf, rg.a0, rg.a1, 0, rg.y0));
      if (rg.y1 < H - 0.01) wallsGroup.add(wallBox(wallMat, wf, rg.a0, rg.a1, rg.y1, H));
      cursor = rg.a1;
    });
  ...
  items.forEach((it) => {
    ...
    if (it.type === "loft") {
      ...
      plat.position.set(cx, D3.LOFT_ELEV - D3.LOFT_T / 2, cz);
      interiorGroup.add(plat);
      const ph = D3.LOFT_ELEV - D3.LOFT_T;
...
function Structure3DViewer({ bldgW, bldgH, items, itemTypes, styleValue, painted, paintBody, paintTrim, frontWall, scale, mgX, mgY, accent, onItemMove, onSnapshot, onClose }) {
  ...
  useEffect(() => {
    ...
    loadThree().then((bundle) => {
      ...
      const bodyColor = painted ? d3CssColor(paintBody, D3_COLORS.body) : D3_COLORS.body;
      const trimColor = painted ? d3CssColor(paintTrim, D3_COLORS.trim) : D3_COLORS.trim;
      const model = buildShed3DModel(THREE, { bldgW, bldgH, items, itemTypes, styleValue, bodyColor, trimColor, frontWall, scale, mgX, mgY });
  ...
  // and interior items stay solid so lofts/workbenches read clearly.
  useEffect(() => {
    const e = engineRef.current;
    if (!e) return;
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(15,23,42,0.9)", zIndex: 1100, display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ display: "flex", gap: 10, alignItems: "center", justifyContent: "center", padding: "12px 16px", background: "#0F172A", flexWrap: "wrap" }}>

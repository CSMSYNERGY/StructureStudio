...
// Used when no ?client= query param is present, OR when the configured client lookup fails.
// New clients should be added to the `client_configs` table in Supabase, not baked in here.
...
// ─── MAIN COMPONENT ───
function StructureStudioInner({ config = DEFAULT_CONFIG }) {
  const C = config;
...
// Resolves the config in this order:
//   1. `config` prop (e.g. supplied by index.html's postMessage re-render handler) wins.
//   2. `?client=<id>` URL param → fetch from Supabase `client_configs.config`.
//   3. Otherwise (and as a fallback on fetch failure) use DEFAULT_CONFIG (Junior Barns).
//
// While the fetch is in flight we render a brief "Loading…" splash so the inner
// component doesn't mount with a stale config and have to re-init from scratch.
export default function StructureStudio({ config: configProp = null }) {
  const [resolvedConfig, setResolvedConfig] = useState(() => {
    ...
    if (!c || c === DEFAULT_CONFIG.clientId) return DEFAULT_CONFIG;
    return null;
  });

  useEffect(() => {
    if (resolvedConfig !== null) return;
    if (typeof window === "undefined") { setResolvedConfig(DEFAULT_CONFIG); return; }
    const c = new URLSearchParams(window.location.search).get("client");
    if (!c) { setResolvedConfig(DEFAULT_CONFIG); return; }
    let cancelled = false;
    (async () => {
      try {
        ...
        if (error || !data?.config) {
          console.warn(`Client "${c}" not found in client_configs — falling back to default.`, error);
          setResolvedConfig(DEFAULT_CONFIG);
        } else {
  ...
  return <StructureStudioInner config={resolvedConfig} />;
}

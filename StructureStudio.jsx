...
function buildShed3DModel(THREE, p) {
  ...
  return { root, wallMat, trimMat, roofGroup, openingsGroup, wallsGroup, interiorGroup };
}
...
function Structure3DViewer({ bldgW, bldgH, items, itemTypes, styleValue, painted, paintBody, paintTrim, frontWall, scale, mgX, mgY, accent, style3d, paintEnabled, onPaintChange, onWallHeight, onItemAdd, onItemMove, onItemSelect, onSnapshot, onClose }) {
  ...
  useEffect(() => {
    ...
    loadThree().then((bundle) => {
      ...
      engineRef.current = { renderer, scene, camera, controls, model, render, resize, ro, applyShellMode, disposeInteraction, setLiveColors, setWallHeight, interior: false };
      resize();
      setPhase("ready");
    }).catch((err) => {

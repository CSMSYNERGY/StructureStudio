...
export default function StructureStudio({ config = DEFAULT_CONFIG }) {
  ...
  // Auto-update building size
  useEffect(() => {
    const p = parseSize(sel.size);
    if (p) { setShedW(p.w); setShedH(p.h); setItems([]); setSelectedId(null); }
  }, [sel.size]);

  const mg = 60;
  const scale = Math.min((560 - mg * 2) / bldgW, (460 - mg * 2) / bldgH);
  const pW = bldgW * scale, pH = bldgH * scale;
  const cW = pW + mg * 2, cH = pH + mg * 2;
  ...
  const handleClick = useCallback((e) => {
    ...
    if (!activeTool) {
      const hit = [...items].reverse().find((it) => {
        ...
        const iw = c.width * scale; const ih = c.height * scale;
    ...
    const wall = getWallFromClick(pt.x, pt.y, pW, pH, mg);
    ...
    if (cfg.doorSnap) {
      ...
      const rampDepth = 3; // 3ft deep
      const rampDepthPx = rampDepth * scale;
      const rampWidthPx = doorW * scale;
      let rx, ry, rot;
      if (closest.wall === "north") { rx = closest.x; ry = mg - rampDepthPx / 2; rot = 0; }
      else if (closest.wall === "south") { rx = closest.x; ry = mg + pH + rampDepthPx / 2; rot = 0; }
      else if (closest.wall === "west") { rx = mg - rampDepthPx / 2; ry = closest.y; rot = 90; }
      else if (closest.wall === "east") { rx = mg + pW + rampDepthPx / 2; ry = closest.y; rot = 90; }
      else return;
    ...
    if (cfg.wallOnly && !wall) return;
    let ni;
    if (cfg.wallSnap) {
      const clickedWall = wall || getNearestWall(pt.x, pt.y, pW, pH, mg);
      const sn = snapToWallInterior(clickedWall, pt.x, pt.y, iwPx, ihPx, pW, pH, mg);
      ...
      const cyFtRound = Math.max(loftH / 2, Math.min(Math.round((pt.y - mg) / scale), bldgH - loftH / 2));
      const cxFt = bldgW / 2;
      ...
      ni = { id: idCounter++, type: "loft", x: mg + cxFt * scale, y: mg + cyFtRound * scale, rotation: 0, wall: null, widthFt: bldgW, heightFt: loftH };
    } else if (wall) {
      const sn = snapToWall(wall, pt.x, pt.y, iwPx, ihPx, pW, pH, mg);
      ni = { id: idCounter++, type: activeTool, ...sn, widthFt: cfg.width, heightFt: cfg.height };
    } else {
      const x = Math.max(mg + iwPx / 2, Math.min(pt.x, mg + pW - iwPx / 2));
      const y = Math.max(mg + ihPx / 2, Math.min(pt.y, mg + pH - ihPx / 2));
      ni = { id: idCounter++, type: activeTool, x, y, rotation: 0, wall: null, widthFt: cfg.width, heightFt: cfg.height };
    }
    setItems((p) => [...p, ni]);
    setActiveTool(null);
    setToast(null);
  }, [activeTool, dragging, getSvgPt, items, mg, pW, pH, scale, ITEMS]);
  ...
  const startResize = useCallback((e, item, handle) => {
    e.preventDefault();
    const pt = getSvgPt(e);
    setResizing({ id: item.id, handle, startPt: pt, origWidthFt: item.widthFt, origHeightFt: item.heightFt, origX: item.x, origY: item.y });
  }, [getSvgPt]);
  ...
  const getResizeBounds = useCallback((item) => {
    ...
    items.forEach((other) => {
      ...
      const oPos = isHoriz ? (other.x - mg) / scale : (other.y - mg) / scale;
      const oLeft = oPos - oW / 2;
      const oRight = oPos + oW / 2;
      
      const itemPos = isHoriz ? (item.x - mg) / scale : (item.y - mg) / scale;
  ...
  }, [items, ITEMS, bldgW, bldgH, mg, scale]);
  ...
  const onPtrMove = useCallback((e) => {
    if (resizing) {
      ...
      if (it.type === "loft") {
        ...
        const mouseXft = Math.round((pt.x - mg) / scale);
        const mouseYft = Math.round((pt.y - mg) / scale);
        const origCxFt = (resizing.origX - mg) / scale;
        const origCyFt = (resizing.origY - mg) / scale;
      ...
      // Workbench: wall-snapped 1D resize
      const isHoriz = it.wall === "north" || it.wall === "south";
      
      // Compute mouse position in feet from wall origin
      const mouseFt = isHoriz ? (pt.x - mg) / scale : (pt.y - mg) / scale;
      const mouseFtSnapped = Math.round(mouseFt);
      
      // Original edges - snap to integer feet
      const origCenterFt = isHoriz ? (resizing.origX - mg) / scale : (resizing.origY - mg) / scale;
      const origLeft = Math.round(origCenterFt - resizing.origWidthFt / 2);
      ...
      if (resizing.handle === "max") {
        // Dragging the high-coordinate edge (right or bottom)
        newRight = Math.max(origLeft + 2, Math.min(mouseFtSnapped, Math.floor(maxEdge)));
      } else {
        // Dragging the low-coordinate edge (left or top)
        newLeft = Math.min(origRight - 2, Math.max(mouseFtSnapped, Math.ceil(minEdge)));
      }
      
      const newWidthFt = newRight - newLeft;
      const newCenterFt = (newLeft + newRight) / 2;
      const newPos = mg + newCenterFt * scale;
    ...
    if (cfg.wallOnly) {
      const w = getWallFromClick(rx, ry, pW, pH, mg);
      if (w) { const sn = snapToWall(w, rx, ry, iWidthFt * scale, cfg.height * scale, pW, pH, mg); setItems((p) => p.map((i) => i.id === dragging.id ? { ...i, ...sn } : i)); }
    } else if (cfg.wallSnap) {
      const nw = getNearestWall(rx, ry, pW, pH, mg);
      const sn = snapToWallInterior(nw, rx, ry, iWidthFt * scale, cfg.height * scale, pW, pH, mg);
      const cand = { ...it, ...sn };
      ...
      let cxFt = (rx - mg) / scale;
      let cyFt = (ry - mg) / scale;
      ...
      if (it.type === "loft") {
        ...
        for (const o of otherLofts) {
          ...
          const oCx2 = (o.x - mg) / scale, oCy2 = (o.y - mg) / scale;
        ...
        const olEdges = otherLofts.map((o) => {
          const ow = (o.widthFt || cfg.width) / 2, oh = (o.heightFt || cfg.height) / 2;
          const ox = (o.x - mg) / scale, oy = (o.y - mg) / scale;
          return { l: ox - ow, r: ox + ow, t: oy - oh, b: oy + oh };
        });
        if (!checkLoftAttached(fL, fR, fT, fB, bldgW, bldgH, olEdges)) {
          setItems((p) => p.map((i) => i.id === dragging.id ? { ...i, x: mg + cxFt * scale, y: mg + cyFt * scale } : i));
          return;
        }
      } else {
        cxFt = Math.max(halfW, Math.min(cxFt, bldgW - halfW));
      ...
      const x = mg + cxFt * scale;
      const y = mg + cyFt * scale;
      setItems((p) => p.map((i) => i.id === dragging.id ? { ...i, x, y } : i));
    }
  }, [dragging, resizing, getSvgPt, items, mg, pW, pH, scale, ITEMS, getResizeBounds]);

  const onPtrUp = useCallback(() => { setDragging(null); setResizing(null); }, []);
  ...
  const rotSel = () => { if (!selectedId) return; setItems((p) => p.map((i) => { if (i.id !== selectedId) return i; const c = ITEMS[i.type]; if (c && (c.wallOnly || c.wallSnap)) return i; return { ...i, rotation: (i.rotation + 90) % 360 }; })); };
  const clearAll = () => { setItems([]); setSelectedId(null); };

  // ─── PNG GENERATION (shared by export & submit) ───
  const generatePNG = () => {
    const canvas = document.createElement("canvas"); const dpr = 2;
    const extraH = 140; // extra height below canvas for customer/options text
    canvas.width = cW * dpr; canvas.height = (cH + extraH) * dpr;
    const ctx = canvas.getContext("2d"); ctx.scale(dpr, dpr);
    ctx.fillStyle = "#FFF"; ctx.fillRect(0, 0, cW, cH + extraH);
    ctx.strokeStyle = "#E8ECF1"; ctx.lineWidth = 0.5;
    for (let fx = 0; fx <= bldgW; fx++) { const x = mg + fx * scale; ctx.beginPath(); ctx.moveTo(x, mg); ctx.lineTo(x, mg + pH); ctx.stroke(); }
    for (let fy = 0; fy <= bldgH; fy++) { const y = mg + fy * scale; ctx.beginPath(); ctx.moveTo(mg, y); ctx.lineTo(mg + pW, y); ctx.stroke(); }
    ctx.strokeStyle = "#1E293B"; ctx.lineWidth = WALL_THICKNESS; ctx.strokeRect(mg, mg, pW, pH);
    ctx.fillStyle = "#475569"; ctx.font = "bold 13px sans-serif"; ctx.textAlign = "center";
    ctx.fillText(`${bldgW} ft`, mg + pW / 2, mg - 14); ctx.fillText(`${bldgW} ft`, mg + pW / 2, mg + pH + 24);
    ctx.save(); ctx.translate(mg - 20, mg + pH / 2); ctx.rotate(-Math.PI / 2); ctx.fillText(`${bldgH} ft`, 0, 0); ctx.restore();
    ctx.save(); ctx.translate(mg + pW + 24, mg + pH / 2); ctx.rotate(Math.PI / 2); ctx.fillText(`${bldgH} ft`, 0, 0); ctx.restore();
    if (frontWall) {
      ctx.fillStyle = "#94A3B8"; ctx.font = "11px sans-serif";
      ctx.fillText(getDisplayLabel("north", frontWall), mg + pW / 2, mg - 30);
      ctx.fillText(getDisplayLabel("south", frontWall), mg + pW / 2, mg + pH + 40);
      ctx.save(); ctx.translate(mg - 38, mg + pH / 2); ctx.rotate(-Math.PI / 2); ctx.fillText(getDisplayLabel("west", frontWall), 0, 0); ctx.restore();
      ctx.save(); ctx.translate(mg + pW + 42, mg + pH / 2); ctx.rotate(Math.PI / 2); ctx.fillText(getDisplayLabel("east", frontWall), 0, 0); ctx.restore();
    }
    ...
    items.forEach((item) => {
      ...
      if (item.type === "loft") {
        ...
        ctx.fillStyle = cfg.color + "30"; ctx.strokeStyle = cfg.color; ctx.lineWidth = 2;
      ...
      if (item.type === "workbench") { ctx.fillText(`${itemW} ft`, 0, 0); ctx.font = "8px sans-serif"; ctx.fillText("Workbench", 0, 12); }
      else if (item.type === "ramp") { ctx.fillText("RAMP", 0, 4); }
      else if (item.type === "loft") { ctx.fillStyle = cfg.color; ctx.fillText("LOFT", 0, 0); ctx.font = "9px sans-serif"; ctx.globalAlpha = 0.7; ctx.fillText(`${itemW}×${itemH} ft`, 0, 13); ctx.globalAlpha = 1; }
      else {
        const lblY = cfg.wallOnly ? ((item.wall === "north" || item.wall === "east") ? 14 : -10) : 4;
        ctx.fillText(cfg.shortLabel, 0, lblY);
      }
      ctx.restore();
    });
    ...
    ctx.textAlign = "left"; ctx.fillText(customerName, mg, textY);
    ctx.textAlign = "right"; ctx.fillText(`${bldgW}×${bldgH}  ${sel.style || ""}`, mg + pW, textY);
    textY += 14;

    // Line 2: Contact info
    ctx.fillStyle = "#64748B"; ctx.font = "10px sans-serif"; ctx.textAlign = "left";
    let infoLine = [contact.phone, contact.email].filter(Boolean).join("  •  ");
    if (infoLine) { ctx.fillText(infoLine, mg, textY); textY += 12; }
    if (customerAddr) { ctx.fillText(customerAddr, mg, textY); textY += 14; }

    // Build bullet list of all placed items + paint
    const bullets = [];
    ...
    items.filter((i) => i.type === "roughOpening").forEach((ro) => {
      const d = (roDimensions[ro.id] || "").trim();
      bullets.push(d ? `Rough Opening — ${d}` : "Rough Opening");
    });
    // Custom options (only those with a name)
    ...
    if (bullets.length > 0) {
      ...
      const colW = (cW - mg * 2) / 2;
      const half = Math.ceil(bullets.length / 2);
      for (let i = 0; i < Math.max(half, bullets.length - half); i++) {
        if (i < half) ctx.fillText("•  " + bullets[i], mg, textY);
        if (i + half < bullets.length) ctx.fillText("•  " + bullets[i + half], mg + colW, textY);
        textY += 13;
      }
    }
  ...
  const exportPNG = () => {
    const url = generatePNG();
    setExportUrl(url); setShowExport(true);
  };
  ...
  const submitQuote = async () => {
    ...
    try {
      const floorPlanImage = generatePNG();
      ...
      const payload = {
        ...
        roughOpenings: items.filter((i) => i.type === "roughOpening").map((ro) => ({
          name: "Rough Opening",
      ...
      const webhookUrl = C.webhookUrl || "https://csmsynergy.app.n8n.cloud/webhook/96317c82-f0af-4f10-b452-6399a196f840";
      ...
      setSubmitError("Something went wrong submitting your quote. Please try again.");
    ...
    <div style={{ fontFamily: "'Segoe UI', system-ui, -apple-system, sans-serif", background: "#F8FAFC", minHeight: "100vh" }}>
        ...
        <div style={{ background: "#FFF", borderBottom: "2px solid #E2E8F0", padding: "14px 20px" }}>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: 10, marginBottom: 10 }}>
              ...
              {C.contactFields.includes("phone") && (
                <div style={{ flex: "1 1 140px" }}>
                  ...
                  <input type="tel" value={contact.phone} onChange={(e) => setContact((p) => ({ ...p, phone: e.target.value }))} placeholder="(555) 555-5555" style={{ ...S.sel, width: "100%", boxSizing: "border-box" }} />
                </div>
              )}
            </div>
              ...
              <div style={{ display: "flex", flexWrap: "wrap", gap: 10 }}>
                {C.contactFields.includes("street") && (
                  <div style={{ flex: "2 1 200px" }}>
                    ...
                    <input type="text" value={contact.street} onChange={(e) => setContact((p) => ({ ...p, street: e.target.value }))} placeholder="123 Main St" style={{ ...S.sel, width: "100%", boxSizing: "border-box" }} />
                  </div>
                )}
                {C.contactFields.includes("city") && (
                  <div style={{ flex: "1 1 130px" }}>
                    <span style={{ ...S.lbl, fontSize: 10, display: "block", marginBottom: 3 }}>City</span>
                    <input type="text" value={contact.city} onChange={(e) => setContact((p) => ({ ...p, city: e.target.value }))} placeholder="City" style={{ ...S.sel, width: "100%", boxSizing: "border-box" }} />
                  </div>
                )}
                {C.contactFields.includes("state") && (
                  <div style={{ flex: "0 1 80px" }}>
                    <span style={{ ...S.lbl, fontSize: 10, display: "block", marginBottom: 3 }}>State</span>
                    <input type="text" value={contact.state} onChange={(e) => setContact((p) => ({ ...p, state: e.target.value }))} placeholder="ST" maxLength={2} style={{ ...S.sel, width: "100%", boxSizing: "border-box", textTransform: "uppercase" }} />
                  </div>
                )}
                {C.contactFields.includes("zip") && (
                  <div style={{ flex: "0 1 100px" }}>
                    <span style={{ ...S.lbl, fontSize: 10, display: "block", marginBottom: 3 }}>Zip</span>
                    <input type="text" value={contact.zip} onChange={(e) => setContact((p) => ({ ...p, zip: e.target.value }))} placeholder="00000" maxLength={10} style={{ ...S.sel, width: "100%", boxSizing: "border-box" }} />
                  </div>
                )}
              </div>
            )}
          ...
          {items.filter((i) => i.type === "roughOpening").length > 0 && (
            <div style={{ marginTop: 14 }}>
              ...
              {items.filter((i) => i.type === "roughOpening").map((ro) => {
                ...
                return (
                  <div key={ro.id} style={{ display: "flex", gap: 6, alignItems: "center", marginBottom: 6 }}>
                    <span style={{ flex: "0 0 auto", fontSize: 12, fontWeight: 600, color: "#334155", minWidth: 110 }}>Rough Opening</span>
      ...
      <div style={{ display: "flex", justifyContent: "center", padding: "16px 20px", background: "#F1F5F9", cursor: activeTool ? "crosshair" : dragging ? "grabbing" : "default" }}>
        <svg ref={svgRef} viewBox={`0 0 ${cW} ${cH}`}
          ...
          onClick={handleClick}>
          <rect x={mg} y={mg} width={pW} height={pH} fill="#FAFBFD" />
          {Array.from({ length: Math.floor(bldgW) + 1 }, (_, i) => <line key={`gx${i}`} x1={mg + i * scale} y1={mg} x2={mg + i * scale} y2={mg + pH} stroke="#E8ECF1" strokeWidth={0.5} />)}
          {Array.from({ length: Math.floor(bldgH) + 1 }, (_, i) => <line key={`gy${i}`} x1={mg} y1={mg + i * scale} x2={mg + pW} y2={mg + i * scale} stroke="#E8ECF1" strokeWidth={0.5} />)}
          <rect x={mg} y={mg} width={pW} height={pH} fill="none" stroke="#1E293B" strokeWidth={WALL_THICKNESS} />
          <text x={mg + pW / 2} y={mg - 16} textAnchor="middle" fill="#475569" fontSize={13} fontWeight="bold">{bldgW} ft</text>
          <text x={mg + pW / 2} y={mg + pH + 26} textAnchor="middle" fill="#475569" fontSize={13} fontWeight="bold">{bldgW} ft</text>
          <text x={mg - 20} y={mg + pH / 2} textAnchor="middle" fill="#475569" fontSize={13} fontWeight="bold" transform={`rotate(-90,${mg - 20},${mg + pH / 2})`}>{bldgH} ft</text>
          <text x={mg + pW + 24} y={mg + pH / 2} textAnchor="middle" fill="#475569" fontSize={13} fontWeight="bold" transform={`rotate(90,${mg + pW + 24},${mg + pH / 2})`}>{bldgH} ft</text>
          {frontWall && (
            <>
              <text x={mg + pW / 2} y={mg - 32} textAnchor="middle" fill="#94A3B8" fontSize={10} fontWeight="600" letterSpacing="0.1em">{getDisplayLabel("north", frontWall)}</text>
              <text x={mg + pW / 2} y={mg + pH + 42} textAnchor="middle" fill="#94A3B8" fontSize={10} fontWeight="600" letterSpacing="0.1em">{getDisplayLabel("south", frontWall)}</text>
              <text x={mg - 38} y={mg + pH / 2} textAnchor="middle" fill="#94A3B8" fontSize={10} fontWeight="600" letterSpacing="0.1em" transform={`rotate(-90,${mg - 38},${mg + pH / 2})`}>{getDisplayLabel("west", frontWall)}</text>
              <text x={mg + pW + 42} y={mg + pH / 2} textAnchor="middle" fill="#94A3B8" fontSize={10} fontWeight="600" letterSpacing="0.1em" transform={`rotate(90,${mg + pW + 42},${mg + pH / 2})`}>{getDisplayLabel("east", frontWall)}</text>
            </>
          )}
          ...
          {items.map((item) => {
            ...
            const itemW = item.widthFt || cfg.width;
            const itemH = item.heightFt || cfg.height;
            const iw = itemW * scale; const ih = itemH * scale; const isSel = item.id === selectedId;
              ...
              <g key={item.id} transform={`translate(${item.x},${item.y}) rotate(${item.rotation})`}
                ...
                {item.type === "loft" ? (
                  <>
                    ...
                    {isSel && (() => {
                      const hz = Math.min(Math.max(ih / 4, 14), 24, ih * 0.4);
                      const vz = Math.min(Math.max(iw / 4, 14), 24, iw * 0.4);
                      return (
                        <>
                  ...
                  <>
                    ...
                    <text x={0} y={(item.wall === "north" || item.wall === "east") ? 14 : -10} textAnchor="middle" fill="#1E293B" fontSize={9} fontWeight="700">{cfg.shortLabel}</text>
                  </>
                ) : (
                  <>
                    <rect x={-iw / 2} y={-ih / 2} width={iw} height={ih} fill={cfg.color + "30"} stroke={cfg.color} strokeWidth={2} rx={2} />
                    {item.type === "ramp" ? (
                      <text x={0} y={4} textAnchor="middle" fill={cfg.color} fontSize={9} fontWeight="700">RAMP</text>
                    ) : isWB ? (
                      <>
      ...
      {submitted && (
        <div style={{ background: "#F0FDF4", borderTop: "2px solid #BBF7D0", padding: "32px 20px", textAlign: "center" }}>
          ...
          <h3 style={{ margin: "0 0 8px", fontSize: 20, fontWeight: 700, color: "#166534" }}>Quote Request Submitted!</h3>
          <p style={{ margin: 0, fontSize: 14, color: "#15803D", maxWidth: 400, marginLeft: "auto", marginRight: "auto" }}>
          ...
          <button
            onClick={() => { setSubmitted(false); setItems([]); setSel((p) => { const n = { ...p }; Object.keys(n).forEach((k) => n[k] = ""); return n; }); setContact({ name: "", phone: "", email: "", street: "", city: "", state: "", zip: "" }); setPaintColors({ body: "", trim: "" }); setCustomOptions([]); setRoDimensions({}); }}
          ...
          <div style={{ background: "#FFF", borderRadius: 16, padding: 24, maxWidth: 580, width: "100%", boxShadow: "0 20px 60px rgba(0,0,0,0.2)" }} onClick={(e) => e.stopPropagation()}>
            ...
            {exportUrl && (
              <>
                ...
                <p style={{ fontSize: 11, color: "#94A3B8", marginTop: 12, textAlign: "center" }}>Download and attach to your GHL estimate or invoice</p>
              </>
            )}
          </div>
        </div>

...
> **Update 2026-08-04 — builder-supplied appearance shipped (see the repo CLAUDE.md §3D for the rules).** Catalog fixtures now render the BUILDER'S OWN door/window photo masked onto the opening (no canned model library — Carolyn's 2026-08-03 direction), with a painted-slab fallback so a catalog door can never render as the empty hole it used to; fixture `heightIn` drives the opening height. The per-style `d3` spec moved to `building_styles.d3`/`.d3_photos` (migration 086) and `get_config` emits it — the old `client_configs.config` path had been dead since 020, so §"four-sided photos" onboarding actually persists now. The calibration editor is reachable by the builder inside the portal (`setup3d` prop → portal-settings actions), and fixture photos get a 4-corner straighten step at upload. Migration 041 is DEAD (floor-plans is PDF-only since 071); photos live in `branding`. Explicitly NOT started: the phone-scan / GLB pipeline (paused pending an app choice — see the vault's StructureStudio 3D Production Plan).
>
> **Status:** ✅ ALL PHASES IMPLEMENTED on branch `beta-2.0` (2026-07-02). Phases 1–4: parametric viewer + quote-PDF snapshot. Phase 5: items are stamped with `openingHeightFt`/`sillFt`/`elevationFt` at placement (3D honors them, D3 fallbacks for legacy designs), wall height is config-driven (`buildingStyles[].wallHeightFt` / top-level `wallHeightFt`, default 8 ft), and `040_wall_height.sql` adds `building_sizes.wall_height_ft` (**NOT YET APPLIED** — hand-apply via SQL Editor). Phase 6: all item classes drag in 3D (doors/windows/ROs along walls, workbenches wall-snapped, lofts free with edge snapping), new wall items place from a 3D palette, selection syncs to 2D, and a **paint swatch picker** recolors the building live and commits to `paintColors`/`sel.paint`. Ramps follow their door in BOTH views (shared `rampPlacementForDoor`), and deleting a door cascades to its ramp. Per-style 3D appearance is config-driven: `d3ResolveStyleSpec` merges the tenant's `buildingStyles[].d3` override ({ roof: { type, pitch, ridgeOffset, overhang, kneeU/kneeRise/ridgeRise }, siding: "batten", colors: { body, trim, roof }, wallHeightFt }) over built-in per-style defaults (`D3_STYLE_DEFAULTS`) — Junior Barns' four styles render distinctly out of the box (econo shed / urban wide-eave gable / northwood steep gable + battens / farmland gambrel + battens + barn tones). Lap-siding upgrade (Carolyn, [2026-07-02 meeting](https://fathom.video/calls/732981007)): `siding: "lap"` renders horizontal lap boards, and `d3SidingOverride` flips the 3D automatically when the customer selects a lap-siding option (explicit `config.siding3d = { optionId, lapValue }` contract, or auto-detected from any option id/value reading "lap siding"). Onboarding note per the same meeting: collect FOUR-SIDED photo sets per building style and tune that style's `d3` spec to match (the IdeaRoom workflow) — now tooled: the designer's `?admin=1` panel has a **3D Style Calibration editor** (per-style spec form + four photo-URL slots + live 3D preview of the draft spec + Copy-JSON + Save via the `save_style_d3` action in admin-save-settings [redeploy required] + "Draft from photos (AI)" via the new `calibrate-style` Edge Function [deploy + ANTHROPIC_API_KEY required]; `041_style_photos.sql` adds owner photo-upload storage policies, NOT YET APPLIED). Customers can also **adjust wall height (6–10 ft) in the 3D view** — rebuilds live, persists as `sel.wallHeight`, ships in the submit payload as `selections.wallHeightFt` (pricing hookup = catalog follow-up). Remaining niceties: portal editor for wall height + style appearance, mobile tap-to-select refinement, texture/GLB asset tier (research report Phase 3).  
